...
console.log('packing azure-code ...');
const cliPackageDir = join('packages', 'cli');
rmSync(join(cliPackageDir, 'dist', 'google-gemini-cli-*.tgz'), { force: true });
execSync(
  `npm pack -w azure-code --pack-destination ./packages/cli/dist`,
  {
    stdio: 'ignore',
  },
);

console.log('packing azure-code-core ...');
const corePackageDir = join('packages', 'core');
rmSync(join(corePackageDir, 'dist', 'google-azure-code-core-*.tgz'), {
  force: true,
});
execSync(
  `npm pack -w azure-code-core --pack-destination ./packages/core/dist`,
  { stdio: 'ignore' },
);
...
chmodSync(
  join(corePackageDir, 'dist', `google-azure-code-core-${packageVersion}.tgz`),
  0o755,
);
...
function buildImage(imageName, dockerfile) {
  ...
  const imageTag =
    process.env.AZURE_SANDBOX_IMAGE_TAG || imageName.split(':')[1];
