...
import { AZURE_DIR } from 'azure-code-core';
...
let geminiSandbox = process.env.AZURE_SANDBOX;

if (!geminiSandbox) {
  const userSettingsFile = join(homedir(), AZURE_DIR, 'settings.json');
...
if (!geminiSandbox) {
  ...
  while (true) {
    const geminiEnv = join(currentDir, AZURE_DIR, '.env');
  ...
  geminiSandbox = process.env.AZURE_SANDBOX;
}
...
if (['1', 'true'].includes(geminiSandbox)) {
  if (commandExists('docker')) {
    ...
    console.error(
      'ERROR: install docker or podman or specify command in AZURE_SANDBOX',
    );
    process.exit(1);
  }
  ...
  if (commandExists(geminiSandbox)) {
    ...
    console.error(
      `ERROR: missing sandbox command '${geminiSandbox}' (from AZURE_SANDBOX)`,
    );
    process.exit(1);
  }
} else {
