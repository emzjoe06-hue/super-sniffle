...
Azure Code creates a copy of the extension during installation. You must run
...
Create a symbolic link between your development directory and Azure Code
...
Azure Code loads extensions from `<home>/.gemini/extensions`. Each extension
...
- `contextFileName`: The name of the file that contains the context for the
  ...
  If this property is not used but a `AZURE.md` file is present in your
...
When Azure Code starts, it loads all the extensions and merges their
...
`commands/` subdirectory. Azure Code uses the directory structure to determine
the command name.
...
Extensions can contribute policy rules and safety checkers to Azure Code
...
place your `.toml` policy files inside it. Azure Code automatically loads all
...
<!-- prettier-ignore -->
> [!WARNING]
> For security, Azure Code ignores any `allow` decisions or `yolo`
...
Azure Code supports variable substitution in `gemini-extension.json` and
`hooks/hooks.json`.
