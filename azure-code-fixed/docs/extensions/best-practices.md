# Azure Code extension best practices
...
Developing extensions for Azure Code is a lightweight, iterative process. Use
...
### Use `AZURE.md` effectively

Your `AZURE.md` file provides essential context to the model.
...
- **Restart the CLI:** Extensions are loaded at the start of a session. Restart
  Azure Code after making changes to the manifest or linking a new extension.
...
- **Test the command:** Run the server's `command` and `args` directly in your
  terminal to ensure it starts correctly outside of Azure Code.
