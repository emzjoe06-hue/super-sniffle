...
Azure Code can integrate with your IDE to provide a more seamless and
...
There are two primary ways to integrate Azure Code with an IDE:

1.  **VS Code companion extension**: Install the "Azure Code Companion"
...
2.  **Agent Client Protocol (ACP)**: An open protocol for interoperability
    ...
    discovery and installation of compatible agents like Azure Code.

## VS Code companion extension

The **Azure Code Companion extension** grants Azure Code direct access to your
...
- **VS Code commands:** You can access Azure Code features directly from the VS
  Code Command Palette (`Cmd+Shift+P` or `Ctrl+Shift+P`):
  - `Azure Code: Run`: Starts a new Azure Code session in the integrated
    terminal.
  - `Azure Code: Accept Diff`: Accepts the changes in the active diff editor.
  - `Azure Code: Close Diff Editor`: Rejects the changes and closes the active
    diff editor.
  - `Azure Code: View Third-Party Notices`: Displays the third-party notices for
    the extension.
...
When you run Azure Code inside a supported editor, it will automatically detect
...
manually, you can run the following command inside Azure Code:
...
<!-- prettier-ignore -->
> [!NOTE]
> The "Azure Code Companion" extension may appear towards the bottom of
...
When enabled, Azure Code will automatically attempt to connect to the IDE
companion extension.
...
- Open the Command Palette and run **Azure Code: Accept Diff**.
...
- Open the Command Palette and run **Azure Code: Close Diff Editor**.
...
Azure Code is officially available in the **ACP Agent Registry**. This allows
you to install and update Azure Code directly within supporting IDEs and
...
Azure Code is an ACP-compatible agent available in the ACP Agent Registry.
...
Any other IDE that supports the ACP Agent Registry can install Azure Code
directly through their in-built registry features.

## Using with sandboxing

If you are using Azure Code within a sandbox, be aware of the following:
...
- **On macOS:** The IDE integration requires network access to communicate with
  ...
  network access.
- **In a Docker container:** If you run Azure Code inside a Docker (or Podman)
...
- **Message:**
  ...
  - **Cause:** Azure Code could not find the necessary environment variables
  ...
  - **Solution:**
    1.  Make sure you have installed the **Azure Code Companion** extension in
...
If automatic IDE detection fails, or if you are running Azure Code in a
...
When this variable is set, Azure Code will skip automatic detection and attempt
to connect using the provided PID.

#### Configuration errors

- **Message:**
  `🔴 Disconnected: Directory mismatch. Azure Code is running in a different location than the open workspace in [IDE Name]. Please run the CLI from one of the following directories: [List of directories]`
...
- **Message:**
  `IDE integration is not supported in your current environment. To use this feature, run Azure Code in one of these supported IDEs: [List of IDEs]`
  - **Cause:** You are running Azure Code in a terminal or environment that is
    not a supported IDE.
  - **Solution:** Run Azure Code from the integrated terminal of a supported
    IDE, like Antigravity or VS Code.

- **Message:**
  `No installer is available for IDE. Please install Azure Code Companion extension manually from the marketplace.`
  - **Cause:** You ran `/ide install`, but the CLI does not have an automated
    installer for your specific IDE.
  - **Solution:** Open your IDE's extension marketplace, search for "Azure Code
    Companion", and
