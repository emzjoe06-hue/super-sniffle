...
Azure Code uses tools to interact with your local environment, access
...
## How to use Azure Code's tools

Tools are generally invoked automatically by Azure Code when it needs to perform
an action. However, you can also trigger specific tools manually using shorthand
syntax.

### Automatic execution and security

When the model wants to use a tool, Azure Code evaluates the request against its
security policies.
...
| [`get_internal_docs`](../tools/internal-docs.md) | `Think` | Accesses Azure Code's own documentation for accurate answers about its capabilities. |
| [`save_memory`](../tools/memory.md)              | `Think` | Persists specific facts and project details to your `AZURE.md` file.                |
...
You can extend Azure Code with custom tools by configuring
