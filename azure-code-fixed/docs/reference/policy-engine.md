...
Azure Code includes a powerful policy engine that provides fine-grained control
...
2.  **Create a new policy file** (for example,
    ...
    priority = 100
    ```
3.  **Run a command** that triggers the policy (for example, ask Azure Code to
...
| :-------- | :--- | :-------------------------------------------------------------------------------- |
| Default   | 1    | Built-in policies that ship with Azure Code.                                      |
...
Azure Code ships with a set of default policies to provide a safe out-of-the-box
experience.
