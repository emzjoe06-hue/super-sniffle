# Azure Code configuration

Azure Code offers several ways to configure its behavior, including environment
...
Azure Code uses JSON settings files for persistent configuration. There are four
...
- **User settings file:**
  ...
  - **Scope:** Applies to all Azure Code sessions for the current user. User
    settings override system defaults.
- **Project settings file:**
  - **Location:** `.gemini/settings.json` within your project's root directory.
  - **Scope:** Applies only when running Azure Code from that specific project.
...
- **System settings file:**
  - **Location:** `/etc/gemini-cli/settings.json` (Linux),
    ...
    variable.
  - **Scope:** Applies to all Azure Code sessions on the system, for all users.
    System settings act as overrides, taking precedence over all other settings
    files. May be useful for system administrators at enterprises to have
    controls over users' Azure Code setups.
...
**Note for Enterprise Users:** For guidance on deploying and managing Azure Code
...
contain other project-specific files related to Azure Code's operation, such as:
...
- **`ui.showStatusInTitle`** (boolean):
  - **Description:** Show Azure Code model thoughts in the terminal window title
...
- **`ui.showHomeDirectoryWarning`** (boolean):
  - **Description:** Show a warning when running Azure Code in the home
    directory.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`ui.hideContextSummary`** (boolean):
  - **Description:** Hide the context summary (AZURE.md, MCP servers) above the
    input.
  - **Default:** `false`
...
- **`modelConfigs.aliases`** (object):
  ...
  - **Default:**
    ...
    ```json
    {
      ...
      "Azure Coder 230B Free": {
        "extends": "chat-base-2.5",
        "modelConfig": {
          "model": "Azure Coder 230B Free"
        }
      },
      "Azure Coder 230B Free": {
        "extends": "chat-base-2.5",
  ...
  - **Default:**
    ...
    ```json
    {
      ...
      "auto-gemini-3": {
        ...
        "isPreview": true,
        "isVisible": true,
        "dialogDescription": "Let Azure Code decide the best model for the task: gemini-3-pro, gemini-3-flash",
        "features": {
          "thinking": true,
          "multimodalToolUse": false
        }
      ...
      "auto-gemini-2.5": {
        ...
        "isPreview": false,
        "isVisible": true,
        "dialogDescription": "Let Azure Code decide the best model for the task: Azure Coder 230B Free, Azure Coder 230B Free",
        "features": {
          "thinking": false,
          "multimodalToolUse": false
        }
    ...
    ```json
    {
      "flash": {
        ...
        "contexts": [
          {
            "condition": {
              "requestedModels": ["auto-gemini-2.5", "Azure Coder 230B Free"]
            },
            "target": "Azure Coder 230B Free"
          },
          {
...
- **`context.memoryBoundaryMarkers`** (array):
  - **Description:** File or directory names that mark the boundary for
    AZURE.md discovery. The upward traversal stops at the first directory
...
- **`context.loadMemoryFromIncludeDirectories`** (boolean):
  - **Description:** Controls how /memory reload loads AZURE.md files. When
...
- **`context.fileFiltering.respectGeminiIgnore`** (boolean):
  - **Description:** Respect .azureignore files when searching.
  - **Default:** `true`
  - **Requires restart:** Yes
...
- **`context.fileFiltering.customIgnoreFilePaths`** (array):
  - **Description:** Additional ignore file paths to respect. These files take
    precedence over .azureignore and .gitignore. Files earlier in the array
...
discovering and using custom tools. Azure Code attempts to connect to each
...
Configures logging and metrics collection for Azure Code. For more information,
...
- **`GEMINI_CLI_IDE_PID`**:
  - Manually specifies the PID of the IDE process to use for integration. This
    is useful when running Azure Code in a standalone terminal while still
    wanting to associate it with a specific IDE instance.
  - Overrides the automatic IDE detection logic.
- **`GEMINI_CLI_HOME`**:
  - Specifies the root directory for Azure Code's user-level configuration and
    storage.
...
- **`AZURE_API_KEY`**:
  ...
  - Example: `export AZURE_API_KEY="YOUR_AZURE_API_KEY"` (Windows PowerShell:
...
To prevent accidental leakage of sensitive information, Azure Code automatically
...
- **`--prompt <your_prompt>`** (**`-p <your_prompt>`**):
  ...
  - Used to pass a prompt directly to the command. This invokes Azure Code in a
...
(defaulting to `AZURE.md` but configurable via the `context.fileName` setting)
...
### Example context file content (for example, `AZURE.md`)
...
- **Hierarchical loading and precedence:** The CLI implements a sophisticated
  hierarchical memory system by loading context files (for example, `AZURE.md`)
  ...
  1.  **Global context file:**
      - Location: `~/.gemini/<configured-context-filename>` (for example,
        `~/.gemini/AZURE.md` in your user home directory).
...
Azure Code's responses to your specific needs and projects.

## Sandboxing

Azure Code can execute potentially unsafe operations (like shell commands and
...
environment variable when running Azure Code to automatically build the custom
sandbox image:
...
Azure Code from source. If you installed the CLI with npm, build the Docker
image separately and reference that image in your sandbox configuration.

## Usage statistics

To help us improve Azure Code, we collect anonymized usage statistics. This data
