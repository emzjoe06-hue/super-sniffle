...
This monorepo contains two main packages: `azure-code` and
`@google/azure-code-core`.

## `azure-code`

This is the main package for Azure Code. It is responsible for the user
...
`@google/azure-code-core`. This means that whether a user installs the package
with `npm install -g azure-code` or runs it directly with
`npx azure-code`, they are using this single, self-contained executable.
...
- **Simplified script execution**: You can run scripts in any package from the
  ...
  `npm run build --workspace azure-code`.
