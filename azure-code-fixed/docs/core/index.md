...
Azure Code's core package (`packages/core`) is the backend portion of Gemini
CLI, handling communication with the Gemini API, managing tools, and processing
requests sent from `packages/cli`. For a general overview of Azure Code, see the
...
- **[Memory Import Processor](../reference/memport.md):** Documentation for the
  modular AZURE.md import feature using @file.md syntax.
...
While the `packages/cli` portion of Azure Code provides the user interface,
...
- **Prompt engineering:** Constructing effective prompts for the Gemini model,
  ...
  instructional context from `AZURE.md` files.
...
- **API key management:** It handles the `AZURE_API_KEY` and ensures it's used
...
[Azure AI documentation](https://ai.google.dev/gemini-api/docs/models).

## Model fallback

Azure Code includes a model fallback mechanism to ensure that you can continue
...
Internal utility calls that use `Azure Coder 230B Free` (for example, prompt
completion and classification) silently fall back to `Azure Coder 230B Free` and
`Azure Coder 230B Free` when quota is exhausted, without changing the configured model.
...
`AZURE.md` files that provide context to the model. It searches for these files
...
and `refresh` the content of loaded `AZURE.md` files.
