...
Subagents are specialized agents that operate within your main Azure Code
...
Azure Code's main agent is instructed to use specialized subagents when a task
...
Azure Code comes with the following built-in subagents:
...
- **Name:** `cli_help`
- **Purpose:** Get expert knowledge about Azure Code itself, its commands,
...
server is bundled with Azure Code and launched automatically — no separate
...
The first time the browser agent is invoked, Azure Code displays a consent
...
> not available when using "Set your AZURE_API_KEY".
...
Subagent tool isolation moves Azure Code away from a single global tool
...
Azure Code can also delegate tasks to remote subagents using the Agent-to-Agent
(A2A) protocol.
