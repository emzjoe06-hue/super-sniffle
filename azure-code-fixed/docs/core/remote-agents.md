...
Azure Code supports connecting to remote subagents using the Agent-to-Agent
(A2A) protocol. This allows Azure Code to interact with other agents, expanding
its capabilities by delegating tasks to remote services.

Azure Code can connect to any compliant A2A agent. You can find samples of A2A
...
Azure Code routes traffic to remote agents through an HTTP/HTTPS proxy if one is
...
Many remote agents require authentication. Azure Code supports several
...
Azure Code supports the following authentication types:
...
use, Azure Code opens your browser for sign-in and persists the resulting tokens
...
When Azure Code loads a remote agent, it validates your auth configuration
...
When connecting to a remote agent, Azure Code first fetches the agent card
...
Users can manage subagents using the following commands within Azure Code:
...
<!-- prettier-ignore -->
> [!TIP]
> You can use the `@cli_help` agent within Azure Code for assistance
