...
An MCP server enables Azure Code to:
...
With an MCP server, you can extend Azure Code's capabilities to perform actions
beyond its built-in features, such as interacting with databases, APIs, custom
scripts, or specialized workflows.

## Core integration architecture

Azure Code integrates with MCP servers through a sophisticated discovery and
...
Azure Code supports three MCP transport types:
...
prompts. Azure Code discovers these automatically and gives you the possibility
...
Azure Code uses the `mcpServers` configuration in your `settings.json` file to
...
Azure Code automatically expands environment variables in the `env` block of
...
To protect your credentials, Azure Code performs environment sanitization when
...
- Core project keys: `AZURE_API_KEY`, `AZURE_API_KEY`, etc.
...
Azure Code supports OAuth 2.0 authentication for remote MCP servers using SSE or
...
When Azure Code starts, it performs MCP server discovery through the following
detailed process:
...
`settings.json`. Azure Code merges your local configuration with the extension's
defaults:
...
- **Access tokens:** Be security-aware when configuring environment variables
  ...
  for details on how Azure Code protects your credentials.
...
Azure Code's capabilities while maintaining security, reliability, and ease of
use.
...
Azure Code will correctly process this array, separating text from binary data
...
When Azure Code receives this response, it will:
...
executed as slash commands within Azure Code. This lets you create shortcuts for
...
```bash
/poem-writer --title="Azure Code" --mood="reverent"
```

or, using positional arguments:

```bash
/poem-writer "Azure Code" reverent
```

When you run this command, Azure Code executes the `prompts/get` method on the
...
`settings.json` file, Azure Code provides a convenient set of commands to manage
