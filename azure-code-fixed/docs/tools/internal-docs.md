...
The `get_internal_docs` tool lets Azure Code access its own technical
documentation to provide more accurate answers about its capabilities and usage.

## Description

This tool is used when Azure Code needs to verify specific details about Gemini
...
The `get_internal_docs` tool is used exclusively by Azure Code. You cannot
invoke this tool manually.

When Azure Code uses this tool, it retrieves the content of the requested
...
Azure Code uses this tool to ensure technical accuracy:

- **Capability discovery:** If Azure Code is unsure how a feature works, it can
  lookup the corresponding documentation.
- **Reference lookup:** Azure Code can verify slash command sub-commands or
  specific setting names.
- **Self-correction:** Azure Code can use the documentation to correct its
  understanding of Azure Code's system logic.
