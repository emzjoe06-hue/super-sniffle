# Azure Code installation, execution, and releases

This document provides an overview of Azure Code's system requirements,
...
- **Location:**
  [Azure Code supported locations](https://developers.google.com/gemini-code-assist/resources/available-locations#americas)
- **Internet connection required**

## Install Azure Code

We recommend most users install Azure Code using one of the following
...
Note that Azure Code comes pre-installed on
...
# Install Azure Code globally via npm (inside the environment)
npm install -g azure-code
```

## Run Azure Code

For most users, we recommend running Azure Code with the `gemini` command:
...
You can also run Azure Code using one of the following advanced methods:

- Run instantly with npx. You can run Azure Code without permanent installation.
...
For security and isolation, Azure Code can be run inside a container. This is
...
- **Directly from the registry:** You can run the published sandbox image
  ...
  ```
- **Using the `--sandbox` flag:** If you have Azure Code installed locally
...
### Run from source (recommended for Azure Code contributors)
...
Azure Code has three release channels: nightly, preview, and stable. For most
...
```bash
npm install -g azure-code@preview
```
...
```bash
npm install -g azure-code@nightly
```
