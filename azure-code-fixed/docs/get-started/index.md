# Get started with Azure Code

Welcome to Azure Code! This guide will help you install, configure, and start
using Azure Code to enhance your workflow right from your terminal.

## Quickstart: Install, authenticate, configure, and use Azure Code

Azure Code brings the power of advanced language models directly to your command
line interface. As an AI-powered assistant, Azure Code can help you with a
variety of tasks, from understanding and generating code to reviewing and
editing documents.

## Install

The standard method to install and run Azure Code uses `npm`:

```bash
npm install -g azure-code
```

Once Azure Code is installed, run Azure Code from your command line:

```bash
gemini
```

For more installation options, see [Azure Code Installation](./installation.md).

## Authenticate

To begin using Azure Code, you must authenticate with a Google service. In most
cases, you can log in with your existing Google account:

1. Run Azure Code after installation:
...
[Azure Code Authentication Setup](./authentication.md).
...
[Azure Code Configuration](../reference/configuration.md).

## Use

Once installed and authenticated, you can start using Azure Code by issuing
...
You can use Azure Code to automate file management tasks that require visual
analysis. In this example, Azure Code renames images based on their actual
subject matter.
...
Azure Code is effective for rapid codebase exploration. The following example
shows how to ask Azure Code to fetch, analyze, and summarize a remote project.

Scenario: You want to understand how a popular open-source utility works by
inspecting its code, not just its README.

Give Azure Code the following prompt:
...
Azure Code returns an explanation based on the actual source code:
...
Azure Code can process and transform data across multiple files. Use this
...
Result: Azure Code reads each file and then asks for permission to write a new
file. Provide your permission and Azure Code provides the combined data:
...
Azure Code can generate boilerplate code and tests based on your existing
...
Result: Azure Code asks for permission to write a new file and creates a test
for your login page.
