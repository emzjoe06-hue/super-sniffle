...
- **Error:
  `You must be a named user on your organization's Azure Code Standard edition subscription to use this service. Please contact your administrator to request an entitlement to Azure Code Standard edition.`**
  - **Cause:** This error might occur if Azure Code detects the
  ...
  - **Solution:**
    ...
    - **Organizational Users:** Contact your Google Cloud administrator to be
      added to your organization's Azure Code subscription.

- **Error:
  `Failed to sign in. Message: Your current account is not eligible... because it is not currently available in your location.`**
  - **Cause:** Azure Code does not currently support your location. For a full
...
- **Error: `Failed to sign in. Message: Request contains an invalid argument`**
  ...
  - **Solution:** For Google Cloud accounts, you can work around this by setting
    ...
    Gemini API key from
    [Azure AI Studio](http://aistudio.google.com/app/apikey), which also
...
- **Error: Command not found (when attempting to run Azure Code with
  `gemini`).**
  - **Cause:** Azure Code is not correctly installed or it is not in your
    system's `PATH`.
  - **Solution:** The update depends on how you installed Azure Code:
    - If you installed `gemini` globally, check that your `npm` global binary
      directory is in your `PATH`. You can update Azure Code using the command
      `npm install -g azure-code@latest`.
    - If you are running `gemini` from source, ensure you are using the correct
      ...
      To update Azure Code, pull the latest changes from the repository, and
...
- **Error: "Operation not permitted", "Permission denied", or similar.**
  - **Cause:** When sandboxing is enabled, Azure Code may attempt operations
...
- **Azure Code is not running in interactive mode in "CI" environments**
  - **Issue:** Azure Code does not enter interactive mode (no prompt appears) if
  ...
  - **Cause:** The `is-in-ci` package checks for the presence of `CI`,
    ...
    non-interactive, which prevents Azure Code from starting in its interactive
    mode.
...
- **Warning: `npm WARN deprecated node-domexception@1.0.0` or
  ...
  - **Issue:** When installing or updating Azure Code globally via
    `npm install -g azure-code` or `npm update -g azure-code`,
  ...
  - **Cause:** These warnings occur because some dependencies (or their
    ...
    versions. Since Azure Code requires Node.js 20 or higher, the platform's
...
Azure Code uses specific exit codes to indicate the reason for termination. This
...
guide_, consider searching Azure Code
