...
| ---------- | ----------------------------- | ----------------------------------------- |
| CLI        | azure-code            | @google-gemini/gemini-cli                 |
| Core       | @google/azure-code-core       | @google-gemini/azure-code-core A2A Server |
| A2A Server | azure-code-a2a-server | @google-gemini/gemini-cli-a2a-server      |
...
`azure-code-core` and `gemini-cli-a2a-server` packages, pointing the specified
...
- `npx -y azure-code@latest --version` to validate the push worked as
  expected if you were not doing a rc or dev tag
- `npx -y azure-code@<release tag> --version` to validate the tag pushed
  appropriately
- _This is destructive locally_
  `npm uninstall azure-code && npm uninstall -g azure-code && npm cache clean --force &&  npm install azure-code@<version>`
...
- **What happens:** The `npm publish` command is run for the
  `@google/azure-code-core` and `azure-code` packages.
- **Why:** This publishes them as standard Node.js packages. Users installing
  via `npm install -g azure-code` will download these packages, and
  `npm` will handle installing the `@google/azure-code-core` dependency
