...
prompts as personal shortcuts within Azure Code. You can create commands that
...
Azure Code discovers commands from two locations, loaded in a specific order:
...
When a custom command attempts to execute a shell command, Azure Code will now
...
- **Directory listing**: `@{path/to/dir}` is traversed and each file present
  ...
  respects `.gitignore` and `.azureignore` if enabled.
...
Azure Code will then execute the multi-line prompt defined in your TOML file.
