...
Azure Code supports a variety of themes to customize its color scheme and
appearance. You can change the theme to suit your preferences via the `/theme`
command or `"theme":` configuration setting.

## Available themes

Azure Code comes with a selection of pre-defined themes, which you can list
using the `/theme` command within Azure Code:
...
Selected themes are saved in Azure Code's
...
Azure Code lets you create your own custom color themes by specifying them in
...
<!-- prettier-ignore -->
> [!WARNING]
> For your safety, Azure Code will only load theme files that
...
- Select your custom theme using the `/theme` command in Azure Code. Your custom
