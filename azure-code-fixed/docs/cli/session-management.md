...
When starting Azure Code, use the `--resume` (or `-r`) flag to load existing
sessions.
...
You can configure how Azure Code manages your session history in your
`settings.json` file. These settings let you control retention policies and
session lengths.

### Session retention

By default, Azure Code automatically cleans up old session data to prevent your
history from growing indefinitely. When a session is deleted, Azure Code also
