...
Azure Code provides built-in support for OpenTelemetry, transforming every agent
...
enterprise-wide usage in the cloud, Azure Code's observability system provides
the actionable intelligence needed to move from "black box" AI to predictable,
high-performance systems.

## OpenTelemetry integration

Azure Code integrates with **[OpenTelemetry]**, a vendor-neutral,
...
2.  Authenticate with Google Cloud using one of these methods:
    ...
    * **Method B: CLI Auth** (Direct export only): Simplest method for local
      users. Azure Code uses the same OAuth credentials you used for login. To
...
1.  Enable telemetry in `.gemini/settings.json`:
    ...
    ```
2.  Run Azure Code and send prompts.
...
After you enable telemetry and run Azure Code, you can view your data in the
...
Azure Code provides a pre-configured
...
"**Azure Code Monitoring**".

![Azure Code Monitoring Dashboard Overview](/docs/assets/monitoring-dashboard-overview.png)

![Azure Code Monitoring Dashboard Metrics](/docs/assets/monitoring-dashboard-metrics.png)

![Azure Code Monitoring Dashboard Logs](/docs/assets/monitoring-dashboard-logs.png)

To learn more, see
[Instant insights: Azure Code’s pre-configured monitoring dashboards](https://cloud.google.com/blog/topics/developers-practitioners/instant-insights-gemini-clis-new-pre-configured-monitoring-dashboards/).
...
Azure Code includes identifiers in its `User-Agent` header to help you
differentiate and report on API traffic from different environments (for
example, identifying calls from Azure Code versus a standard terminal).
...
| :---------------------------------- | :--------------------------- | :---------- |
| **Azure Code (Agent Mode)** | `GeminiCLI-a2a-server`       | `vscode`    |
...
Azure Code includes `session.id`, `installation.id`, `active_approval_mode`, and
`user.email` (when authenticated) as common attributes on all data.

### Logs

Logs provide timestamped records of specific events. Azure Code logs events
...
These logs track how Azure Code selects and routes requests to models.
...
Azure Code exports several custom metrics.
...
Azure Code provides detailed performance metrics for advanced monitoring.
