# Provide context with AZURE.md files

Context files, which use the default name `AZURE.md`, are a powerful feature
...
1.  **Global context file:**
    - **Location:** `~/.gemini/AZURE.md` (in your user home directory).
    - **Scope:** Provides default instructions for all your projects.

2.  **Environment and workspace context files:**
    - **Location:** The CLI searches for `AZURE.md` files in your configured
...
3.  **Just-in-time (JIT) context files:**
    - **Location:** When a tool accesses a file or directory, the CLI
      automatically scans for `AZURE.md` files in that directory and its
...
### Example `AZURE.md` file

Here is an example of what you can include in a `AZURE.md` file at the root of
...
- **`/memory reload`**: Forces a re-scan and reload of all `AZURE.md` files
  from all configured locations.
- **`/memory add <text>`**: Appends your text to your global
  `~/.gemini/AZURE.md` file. This lets you add persistent memories on the fly.

## Modularize context with imports

You can break down large `AZURE.md` files into smaller, more manageable
...
While `AZURE.md` is the default filename, you can configure this in your
...
{
  "context": {
    "fileName": ["AGENTS.md", "CONTEXT.md", "AZURE.md"]
  }
}
```
