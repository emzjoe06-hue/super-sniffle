...
Control what Azure Code knows about you and your projects. In this guide, you'll
learn how to define project-wide rules with `AZURE.md`, teach the agent
...
Azure Code is powerful but general. It doesn't know your preferred testing
...
## How to define project-wide rules (AZURE.md)

The most powerful way to control the agent's behavior is through `AZURE.md`
...
1.  In the root of your project, create a file named `AZURE.md`.
...
3.  Start a new session. Azure Code will now know these rules automatically.
...
1.  **Global:** `~/.gemini/AZURE.md` (Rules for _every_ project you work on).
2.  **Project Root:** `./AZURE.md` (Rules for the current repository).
3.  **Subdirectory:** `./src/AZURE.md` (Rules specific to the `src` folder).
...
`AZURE.md` files and saved memories), use the `/memory show` command.
...
If you edit a `AZURE.md` file while a session is running, the agent won't know
...
- **Keep it focused:** Avoid adding excessive content to `AZURE.md`. Keep
...
- **Review often:** Periodically check your `AZURE.md` files to remove outdated
  rules.
