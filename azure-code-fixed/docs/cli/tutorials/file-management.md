# File management with Azure Code

Explore, analyze, and modify your codebase using Azure Code. In this guide,
you'll learn how to provide Azure Code with files and directories, modify and
create files, and control what Azure Code can see.

## Prerequisites

- Azure Code installed and authenticated.
- A project directory to work with (for example, a git repository).

## Providing context by reading files

Azure Code will generally try to read relevant files, sometimes prompting you
for access (depending on your settings). To ensure that Azure Code uses a file,
...
If you _don't_ know the exact file path, you can ask Azure Code to find it for
...
Once Azure Code has context, you can direct it to make specific edits. The agent
...
Azure Code uses the `replace` tool to propose a targeted code change.
...
Azure Code uses the `write_file` tool to generate the new file from scratch.

## Review and confirm changes

Azure Code prioritizes safety. Before any file is modified, it presents a
...
Azure Code uses the `run_shell_command` tool to execute your test runner (for
example, `npm test` or `jest`). This ensures the changes didn't break existing
functionality.

## Advanced: Controlling what Gemini sees

By default, Azure Code respects your `.gitignore` file. It won't read or search
...
`.azureignore` file in your project root.
