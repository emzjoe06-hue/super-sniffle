...
Automate tasks with Azure Code. Learn how to use headless mode, pipe data into
Azure Code, automate workflows with shell scripts, and generate structured JSON
...
Headless mode runs Azure Code once and exits. It's perfect for:
...
Run Azure Code in headless mode by providing a prompt with the `-p` (or
...
## How to pipe input to Azure Code
...
## Use Azure Code output in scripts
...
1.  Save the following code as `generate_docs.sh` (or `generate_docs.ps1` for
    ...
    **macOS/Linux (`generate_docs.sh`)**
    ...
    for file in *.py; do
      ...
      # Ask Azure Code to generate the documentation and print it to stdout
...
4.  Use your new command:
    ...
    Azure Code will analyze your staged changes and commit them with a generated
    message.
