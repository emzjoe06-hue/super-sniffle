...
You can either configure Azure Code to start in Plan Mode by default or enter
Plan Mode manually during a session.

### Launch in Plan Mode

To start Azure Code directly in Plan Mode by default:

1.  Use the `/settings` command.
2.  Set **Default Approval Mode** to `Plan`.

To launch Azure Code in Plan Mode once:

1. Use `gemini --approval-mode=plan` when launching Azure Code.

### Enter Plan Mode manually

To start Plan Mode while using Azure Code:

- **Keyboard shortcut:** Press `Shift+Tab` to cycle through approval modes
  (`Default` -> `Auto-Edit` -> `Plan`). Plan Mode is automatically removed from
  the rotation when Azure Code is actively processing or showing confirmation
  dialogs.
...
- **Natural Language:** Ask Azure Code to "start a plan for...". Azure Code
  calls the
  [`enter_plan_mode`](../tools/planning.md#1-enter_plan_mode-enterplanmode) tool
  to switch modes. This tool is not available when Azure Code is in
  [YOLO mode](../reference/configuration.md#command-line-arguments).

## How to use Plan Mode

Plan Mode lets you collaborate with Azure Code to design a solution before
Azure Code takes action.

1.  **Provide a goal:** Start by describing what you want to achieve. Azure Code
    will then enter Plan Mode (if it's not already) to research the task.
2.  **Discuss and agree on strategy:** As Azure Code analyzes your codebase, it
    ...
    options using [`ask_user`](../tools/ask-user.md). **Azure Code will stop and
...
3.  **Review the plan:** Once you've agreed on the strategy, Azure Code creates
...
4.  **Approve or iterate:** Azure Code will present the finalized plan for your
    formal approval.
...
You can collaborate with Azure Code by making direct changes or leaving comments
in the implementation plan. This is often faster and more precise than
describing complex changes in natural language.

1.  **Open the plan:** Press `Ctrl+X` when Azure Code presents a plan for
    review.
...
4.  **Review and refine:** Azure Code automatically detects the changes, reviews
...
- **Approve a plan:** When Azure Code presents a finalized plan, approving it
...
- **Natural language:** Ask Azure Code to "exit plan mode" or "stop planning."
...
workflows. You can customize how Azure Code plans by using skills, adjusting
safety policies, changing where plans are stored, or adding hooks.

### Custom planning with skills

You can use [Agent Skills](../cli/skills.md) to customize how Azure Code
...
- A **"Security Audit"** skill could prompt Azure Code to look for specific
  vulnerabilities during codebase exploration.
- A **"Frontend Design"** skill could guide Azure Code to use specific UI
  components and accessibility standards in its proposal.

To use a skill in Plan Mode, you can explicitly ask Azure Code to "use the
`<skill-name>` skill to plan..." or Azure Code may autonomously activate it
...
Tell Azure Code it can use these tools in your prompt, for example: _"You can
...
the planning workflow or enforce additional checks when Azure Code transitions
...
Storage whenever Azure Code exits Plan Mode to start the implementation.
...
how Azure Code researches and stores plans for your specific domain.
...
When using an [auto model](../reference/configuration.md#model), Azure Code
...
By default, Azure Code automatically cleans up old session data, including all
...
When running Azure Code in non-interactive environments (such as headless
...
- **Automated implementation:** When exiting Plan Mode to execute the plan,
  Azure Code automatically switches to
