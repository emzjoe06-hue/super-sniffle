# Sandboxing in Azure Code

This document provides a guide to sandboxing in Azure Code, including
prerequisites, quickstart, and configuration.

## Prerequisites

Before using sandboxing, you need to install and set up Azure Code:
...
When you set `sandbox: "runsc"`, Azure Code runs
...
- A container must be created and running before starting Azure Code. Gemini
...
(like `shell_exec` and `write_file`) instead of sandboxing the entire Azure Code
process.
...
Sandbox expansion is a dynamic permission system that lets Azure Code request
...
requiring extra permissions (like `npm install`), Azure Code will present you
with a "Sandbox Expansion Request."

### How sandbox expansion works

1.  **Detection**: Azure Code detects a sandbox denial or proactively identifies
...
- Add to a custom Dockerfile. Automatic `BUILD_SANDBOX` builds are only
  available when running Azure Code from source; npm installs need a prebuilt
  image instead.
