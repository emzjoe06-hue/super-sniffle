...
Azure Code includes a model routing feature that automatically switches to a
...
2.  **User consent:** Depending on the failure and the model's policy, the CLI
    ...
    use a silent fallback chain for `Azure Coder 230B Free` and will fall back
    to `Azure Coder 230B Free` and `Azure Coder 230B Free` without prompting or changing the
    configured model.
...
Azure Code supports using a local model for routing decisions. When configured,
Azure Code will use a locally-running **Gemma** model to make routing decisions
...
The model used by Azure Code is determined by the following order of precedence:
