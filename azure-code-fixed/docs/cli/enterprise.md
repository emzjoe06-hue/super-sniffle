# Azure Code for the enterprise

This document outlines configuration patterns and best practices for deploying
and managing Azure Code in an enterprise environment. By leveraging system-level
...
> Azure Code. However, they should not be considered a foolproof security
...
that appears earlier in the user's `PATH` than the actual Azure Code binary (for
...
# Pass all arguments to the real Azure Code executable.
...
with the actual Azure Code process, which inherits the environment variable.
...
servers), you can isolate Azure Code state by overriding the user's home
directory.

By default, Azure Code stores configuration and history in `~/.gemini`. You can
...
Azure Code loads `settings.json` files from three levels: System, Workspace, and
...
For auditing and monitoring purposes, you can configure Azure Code to send
...
control that is configured on a proxy server, not within Azure Code itself. It
