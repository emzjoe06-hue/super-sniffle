...
Hooks are scripts or programs that Azure Code executes at specific points in the
...
Azure Code waits for all matching hooks to complete before continuing.
...
Hooks are triggered by specific events in Azure Code's lifecycle.
...
3. **Debug via Stderr**: Use `stderr` for **all** logging and debugging (for
   example, `echo "debug" >&2`). Azure Code captures `stderr` but never attempts
   to parse it as JSON.

#### Exit codes

Azure Code uses exit codes to determine the high-level outcome of a hook
execution:
...
Hooks are configured in `settings.json`. Azure Code merges configurations from
...
Azure Code **fingerprints** project hooks. If a hook's name or command changes
