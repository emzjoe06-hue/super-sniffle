...
Azure Code uses exit codes for high-level flow control:
...
1. **Detection**: Azure Code detects the hooks.
...
> changed (for example, by a `git pull`), its identity changes. Azure Code will
...
| **Data Exfiltration**        | A hook could read your input (prompts), output (code), or environment variables (`AZURE_API_KEY`) and send them to a remote server. |
...
Hooks inherit the environment of Azure Code process, which may include sensitive
API keys. Azure Code provides a
...
Prevent denial-of-service (hanging agents) by enforcing timeouts. Azure Code
