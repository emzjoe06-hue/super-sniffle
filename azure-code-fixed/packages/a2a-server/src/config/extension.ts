...
export const EXTENSIONS_DIRECTORY_NAME = path.join(AZURE_DIR, 'extensions');
 ...
 * AzureCodeExtension class defined in Core.
 */
...
export function loadExtensions(workspaceDir: string): AzureCodeExtension[] {
  const allExtensions = [
    ...loadExtensionsFromDir(workspaceDir),
    ...loadExtensionsFromDir(homedir()),
  ];

  const uniqueExtensions: AzureCodeExtension[] = [];
...
function loadExtensionsFromDir(dir: string): AzureCodeExtension[] {
  const extensionsDir = path.join(dir, EXTENSIONS_DIRECTORY_NAME);
  if (!fs.existsSync(extensionsDir)) {
    return [];
  }

  const extensions: AzureCodeExtension[] = [];
...
function loadExtension(extensionDir: string): AzureCodeExtension | null {
