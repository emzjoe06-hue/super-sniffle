...
export async function loadConfig(
  ...
  const configParams: ConfigParameters = {
    ...
    fileFiltering: {
      ...
      respectAzureIgnore: settings.fileFiltering?.respectAzureIgnore,
  ...
  const fileService = new FileDiscoveryService(workspaceDir, {
    ...
    respectAzureIgnore: configParams?.fileFiltering?.respectAzureIgnore,
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    // prefer gemini-specific .env under AZURE_DIR
    const geminiEnvPath = path.join(currentDir, AZURE_DIR, '.env');
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homeGeminiEnvPath = path.join(process.cwd(), AZURE_DIR, '.env');
...
async function refreshAuthentication(
    ...
    try {
      ...
      if (isHeadless || useComputeAdc) {
        const reason = isHeadless
          ? 'headless mode'
          : 'AZURE_CODE_USE_COMPUTE_ADC=true';
  ...
  } else if (process.env['AZURE_API_KEY']) {
    logger.info(`[${logPrefix}] Using Gemini API Key`);
    await config.refreshAuth(AuthType.USE_AZURE);
  } else {
    const errorMessage = `[${logPrefix}] Unable to set GeneratorConfig. Please provide a AZURE_API_KEY or set USE_CCPA.`;
