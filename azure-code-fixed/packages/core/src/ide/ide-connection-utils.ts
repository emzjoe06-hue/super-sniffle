...
export function validateWorkspacePath(
  ...
  if (!isWithinWorkspace) {
    return {
      isValid: false,
      error: `Directory mismatch. Azure is running in a different location than the open workspace in the IDE. Please run the CLI from one of the following directories: ${ideWorkspacePaths.join(
        ', ',
      )}`,
    };
  }
...
> {
  ...
  const portFileDir = path.join(os.tmpdir(), 'azure', 'ide');
  let portFiles;
  try {
