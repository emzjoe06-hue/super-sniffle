...
import type { FunctionCall } from '../core/azureApiClient.js';
...
describe('policy_enforcer', () => {
  ...
  it('should return ALLOW when content generator returns ALLOW', async () => {
    ...
    const toolCall: { name: string; args: Record<string, unknown> } = { name: 'testTool', args: {} };
    const policy = {
      testTool: {
  ...
  it('should handle empty policy by checking with LLM (fail-open/check behavior)', async () => {
    ...
    const toolCall: { name: string; args: Record<string, unknown> } = { name: 'unknownTool', args: {} };
