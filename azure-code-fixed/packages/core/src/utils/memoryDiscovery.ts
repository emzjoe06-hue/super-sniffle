...
import { AZURE_DIR, homedir, normalizePath, isSubpath } from './paths.js';
...
async function getGeminiMdFilePathsInternalForEachDir(
  ...
  for (const geminiMdFilename of geminiMdFilenames) {
    ...
    const globalGeminiDir = normalizePath(path.join(resolvedHome, AZURE_DIR));
...
export async function getGlobalMemoryPaths(): Promise<string[]> {
  ...
  const accessChecks = geminiMdFilenames.map(async (filename) => {
    const globalPath = normalizePath(path.join(userHome, AZURE_DIR, filename));
    try {
...
/**
 * Traverses upward from startDir to stopDir, finding all AZURE.md variants.
 *
...
async function findUpwardGeminiFiles(
  ...
  const geminiMdFilenames = getAllGeminiMdFilenames();
  const globalGeminiDir = normalizePath(path.join(homedir(), AZURE_DIR));
...
/**
 * Loads hierarchical AZURE.md files and concatenates their content.
...
export async function loadServerHierarchicalMemory(
  ...
  if (allFilePathsStringDeduped.length === 0) {
    debugLogger.debug(
      '[DEBUG] [MemoryDiscovery] No AZURE.md files found in hierarchy of the workspace.',
    );
    return {
  ...
  if (allFilePaths.length === 0) {
    debugLogger.debug(
      '[DEBUG] [MemoryDiscovery] No unique AZURE.md files found after deduplication by file identity.',
    );
    return {
      memoryContent: { global: '', extension: '', project: '' },
      fileCount: 0,
...
export async function loadJitSubdirectoryMemory(
  ...
  // path like /app/src/file.ts/AZURE.md.
