...
import { AZURE_DIR } from './paths.js';
import { AZURE_IGNORE_FILE_NAME } from 'src/config/constants.js';
...
`.trim();
  ...
  describe('with gitignore', () => {
    ...
    it('should ignore files and folders specified in .gitignore', async () => {
      ...
      await createTestFile(AZURE_DIR, 'config.yaml');
      await createTestFile(AZURE_DIR, 'logs.json');
  ...
  describe('with azureignore', () => {
    it('should ignore azureignore files by default', async () => {
      await fsPromises.writeFile(
    ...
    it('should not ignore files if respectAzureIgnore is false', async () => {
      await fsPromises.writeFile(
        path.join(testRootDir, AZURE_IGNORE_FILE_NAME),
        'ignored.txt\nnode_modules/\n.gemini/\n!/.gemini/config.yaml',
      );
