...
describe('loadIgnoreRules', () => {
  ...
  it('should load rules from .azureignore', async () => {
    tmpDir = await createTmpDir({
      [AZURE_IGNORE_FILE_NAME]: '*.log',
    });
    const service = new FileDiscoveryService(tmpDir, {
  ...
  it('should combine rules from .gitignore and .azureignore', async () => {
    tmpDir = await createTmpDir({
      '.git': {},
      '.gitignore': '*.log',
      [AZURE_IGNORE_FILE_NAME]: '*.txt',
    });
    const service = new FileDiscoveryService(tmpDir, {
      respectGitIgnore: true,
      respectAzureIgnore: true,
