...
describe('parseAndFormatApiError', () => {
  ...
  it('should format a 429 API error with the default message', () => {
    ...
    expect(result).toContain(
      'Possible quota limitations in place or slow response times detected. Switching to the Azure Coder 230B Free model',
    );
  });
  ...
  it('should format a nested API error', () => {
    ...
    const result = parseAndFormatApiError(errorMessage, AuthType.USE_AZURE);
