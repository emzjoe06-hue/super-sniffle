...
import { ApiError } from '../core/azureApiClient.js';
...
describe('retryWithBackoff', () => {
  ...
  describe('Flash model fallback for OAuth users', () => {
    it('should trigger fallback for OAuth personal users on TerminalQuotaError', async () => {
      const fallbackCallback = vi.fn().mockResolvedValue('Azure Coder 230B Free');
    ...
    it.each([[AuthType.USE_AZURE], [AuthType.USE_VERTEX_AI], [undefined]])(
