...
import type { Config, AzureCodeExtension } from '../config/config.js';
...
export abstract class ExtensionLoader {
   ...
   */
  protected async startExtension(extension: AzureCodeExtension) {
    if (!this.config) {
   ...
   * initialized.
   */
  protected async stopExtension(extension: AzureCodeExtension) {
    if (!this.config) {
  ...
  async restartExtension(extension: AzureCodeExtension): Promise<void> {
    await this.stopExtension(extension);
    await this.startExtension(extension);
  }
}
...
export class SimpleExtensionLoader extends ExtensionLoader {
  constructor(
    protected readonly extensions: AzureCodeExtension[],
  ...
  async loadExtension(extension: AzureCodeExtension) {
  ...
  async unloadExtension(extension: AzureCodeExtension) {
