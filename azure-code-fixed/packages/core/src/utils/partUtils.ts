...
import type {
  ...
  Part,
} from '../core/azureApiClient.js';

/**
 * Converts a Part[] into a string.
...
/**
 * Asynchronously maps over a Part[], applying a transformation function
 * to the text content of each text-based part.
 *
 * @param parts The Part[] to process.
 * @param transform A function that takes a string of text and returns a Promise
 *   resolving to an array of new Parts.
 * @returns A Promise that resolves to a new array of Parts with the
 *   transformations applied.
 */
export async function flatMapTextParts(
  parts: Part[],
  transform: (text: string) => Promise<Part[]>,
): Promise<Part[]> {
  const result: Part[] = [];
...
export function appendToLastTextPart(
  ...
  if (typeof lastPart === 'string') {
    // This case should not happen since lastPart is a Part, not a string
    // But handle it anyway by pushing a new text part
    newPrompt[newPrompt.length - 1] = { text: `${lastPart}${separator}${textToAppend}` };
