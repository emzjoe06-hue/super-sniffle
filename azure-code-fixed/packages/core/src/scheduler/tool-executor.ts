  ...
  private async truncateOutputIfNeeded(
    ...
    if (typeof content === 'string' && toolName === SHELL_TOOL_NAME) {
      ...
      if (threshold > 0 && content.length > threshold) {
        ...
        const truncatedStr = formatTruncatedToolOutput(
          content,
          outputFile,
          threshold,
        );
        const truncatedContent: Part[] = [{ text: truncatedStr }];
        ...
        logToolOutputTruncated(
          ...
          new ToolOutputTruncatedEvent(call.request.prompt_id, {
            ...
            truncatedContentLength: truncatedStr.length,
            threshold,
          }),
        );
