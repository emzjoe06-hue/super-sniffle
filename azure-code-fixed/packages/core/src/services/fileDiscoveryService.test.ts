...
import { AZURE_IGNORE_FILE_NAME } from '../config/constants.js';
...
describe('FileDiscoveryService', () => {
  ...
  describe('initialization', () => {
    ...
    it('should load .azureignore patterns even when not in a git repo', async () => {
      await createTestFile(AZURE_IGNORE_FILE_NAME, 'secrets.txt');
      const service = new FileDiscoveryService(projectRoot);
    ...
    it('should correctly resolve options passed to constructor', () => {
      ...
      expect(defaults.respectAzureIgnore).toBe(false);
    ...
    it('should use defaults when options are not provided', () => {
      ...
      expect(defaults.respectAzureIgnore).toBe(true);
  ...
  describe('filterFiles', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/\n.git/\ndist');
      await createTestFile(AZURE_IGNORE_FILE_NAME, 'logs/');
    });

    it('should filter out git-ignored and azure-ignored files by default', () => {
      const files = [
        'src/index.ts',
    ...
    it('should not filter files when respectGitIgnore is false', () => {
      ...
      const filtered = service.filterFiles(files, {
        respectGitIgnore: false,
        respectAzureIgnore: true, // still respect this one
      });
    ...
    it('should not filter files when respectAzureIgnore is false', () => {
      const files = [
        'src/index.ts',
        'node_modules/package/index.js',
        'logs/latest.log',
  ...
  describe('filterFilesWithReport', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile(AZURE_IGNORE_FILE_NAME, '*.log');
    });
  ...
  describe('shouldIgnoreFile & shouldIgnoreDirectory', () => {
    ...
    it('should return true for azure-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);
    ...
    it('should return false for non-azure-ignored files', () => {
      const service = new FileDiscoveryService(projectRoot);
  ...
  describe('precedence (.azureignore over .gitignore)', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
    });

    it('should un-ignore a file in .azureignore that is ignored in .gitignore', async () => {
      await createTestFile('.gitignore', '*.txt');
      await createTestFile(AZURE_IGNORE_FILE_NAME, '!important.txt');
    ...
    it('should un-ignore a directory in .azureignore that is ignored in .gitignore', async () => {
    ...
    it('should extend ignore rules in .azureignore', async () => {
      await createTestFile('.gitignore', '*.log');
      await createTestFile(AZURE_IGNORE_FILE_NAME, 'temp/');
    ...
    it('should use .gitignore rules if respectAzureIgnore is false', async () => {
      await createTestFile('.gitignore', '*.txt');
      await createTestFile(AZURE_IGNORE_FILE_NAME, '!important.txt');
    ...
    it('should use .azureignore rules if respectGitIgnore is false', async () => {
      await createTestFile('.gitignore', '*.txt');
      await createTestFile(AZURE_IGNORE_FILE_NAME, '!important.txt\ntemp/');
      ...
      // .azureignore un-ignores important.txt (which wasn't ignored anyway)
      // and ignores temp/
      expect(filtered).toEqual(
  ...
  describe('custom ignore file', () => {
    ...
    it('should prioritize custom ignore patterns over .azureignore patterns in git repo', async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', 'node_modules/');
      await createTestFile(AZURE_IGNORE_FILE_NAME, '*.log');

      const customIgnoreName = '.customignore';
      // .azureignore ignores *.log, custom un-ignores debug.log
    ...
    it('should prioritize custom ignore patterns over .azureignore patterns in non-git repo', async () => {
      // No .git directory created
      await createTestFile(AZURE_IGNORE_FILE_NAME, 'secret.txt');

      const customIgnoreName = '.customignore';
      // .azureignore ignores secret.txt, custom un-ignores it
  ...
  describe('getIgnoreFilePaths & getAllIgnoreFilePaths', () => {
    beforeEach(async () => {
      await fs.mkdir(path.join(projectRoot, '.git'));
      await createTestFile('.gitignore', '*.log');
      await createTestFile(AZURE_IGNORE_FILE_NAME, '*.tmp');
      await createTestFile('.customignore', '*.secret');
    });

    it('should return .azureignore path by default', () => {
      const service = new FileDiscoveryService(projectRoot);
      const paths = service.getIgnoreFilePaths();
      expect(paths).toEqual([path.join(projectRoot, AZURE_IGNORE_FILE_NAME)]);
    });

    it('should not return .azureignore path if respectAzureIgnore is false', () => {
      const service = new FileDiscoveryService(projectRoot, {
        respectAzureIgnore: false,
      });
      const paths = service.getIgnoreFilePaths();
    ...
    it('should return custom ignore file paths', () => {
      ...
      const paths = service.getIgnoreFilePaths();
      expect(paths).toContain(path.join(projectRoot, AZURE_IGNORE_FILE_NAME));
  ...
  describe('getIgnoredPaths', () => {
    ...
    it('should return all ignored paths that exist on disk', async () => {
      ...
      await createTestFile(AZURE_IGNORE_FILE_NAME, 'secrets/');
    ...
    it('should respect FilterFilesOptions when provided', async () => {
      ...
      await createTestFile(AZURE_IGNORE_FILE_NAME, 'ignored-by-gemini.txt');
