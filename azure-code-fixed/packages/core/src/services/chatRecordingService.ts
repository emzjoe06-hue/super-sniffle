      ...
      try {
        ...
        if (isRewindRecord(record)) {
          ...
          if (
            ...
            (record.type === 'user' || record.type === 'azure')
          ) {
...
export class ChatRecordingService {
  ...
  recordMessageTokens(
    respUsageMetadata: UsageMetadata,
  ): void {
    if (!this.conversationFile || !this.cachedConversation) return;
    ...
    try {
      ...
      if (lastMsg && lastMsg.type === 'azure' && !lastMsg.tokens) {
    ...
    try {
      ...
      if (
        ...
        const newMsg: MessageRecord = {
          ...this.newMessage('azure' as const, [{ text: '' }]),
          type: 'azure' as const,
  ...
  updateMessagesFromHistory(history: readonly Content[]): void {
    ...
    try {
      ...
      for (const message of this.cachedConversation.messages) {
        ...
        if (message.type === 'azure' && message.toolCalls) {
