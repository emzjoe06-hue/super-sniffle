...
import { GoogleGenAI } from './azureApiClient.js';
...
describe('createContentGenerator', () => {
      ...
      httpOptions: expect.objectContaining({
        headers: expect.objectContaining({
          'User-Agent': expect.stringMatching(
            /AzureCode\/1\.2\.3\/gemini-pro \(.*; .*; terminal\)/,
          ),
        }),
      }),
    });
  ...
  it('should use standard User-Agent for a2a-server running outside VS Code', async () => {
    ...
    await createContentGenerator(
      { apiKey: 'test-api-key', authType: AuthType.USE_AZURE },
      mockConfig,
      undefined,
    );
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /AzureCode-a2a-server\/1\.2\.3\/gemini-pro \(.*; .*; terminal\)/,
            ),
          }),
        }),
      }),
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /CloudCodeVSCode\/1\.2\.3 \(aidev_client; os_type=.*; os_version=.*; arch=.*; host_path=VSCode\/1\.85\.0; proxy_client=azurecode\)/,
            ),
          }),
        }),
      }),
    ...
    expect(GoogleGenAI).toHaveBeenCalledWith(
      expect.objectContaining({
        httpOptions: expect.objectContaining({
          headers: expect.objectContaining({
            'User-Agent': expect.stringMatching(
              /AzureCode-my-client\/1\.2\.3\/gemini-pro \(.*; .*; terminal\)/,
            ),
          }),
        }),
      }),
  ...
  it('should allow custom headers to override User-Agent', async () => {
    ...
    vi.stubEnv('AZURE_CODE_CUSTOM_HEADERS', 'User-Agent:MyCustomUA');
  ...
  it('should include custom headers from AZURE_CODE_CUSTOM_HEADERS for Code Assist requests', async () => {
    const mockGenerator = {} as unknown as ContentGenerator;
    vi.mocked(createCodeAssistContentGenerator).mockResolvedValue(
      mockGenerator as never,
    );
    vi.stubEnv(
      'AZURE_CODE_CUSTOM_HEADERS',
  ...
  it('should include custom headers from AZURE_CODE_CUSTOM_HEADERS for GoogleGenAI requests without inferring auth mechanism', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
  ...
  it('should pass api key as Authorization Header when AZURE_API_KEY_AUTH_MECHANISM is set to bearer', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('AZURE_API_KEY_AUTH_MECHANISM', 'bearer');
  ...
  it('should not pass api key as Authorization Header when AZURE_API_KEY_AUTH_MECHANISM is not set (default behavior)', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    // AZURE_API_KEY_AUTH_MECHANISM is not stubbed, so it will be undefined, triggering default 'x-goog-api-key'
  ...
  it('should pass apiVersion to GoogleGenAI when AZURE_CODE_API_VERSION is set', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('AZURE_CODE_API_VERSION', 'v1');
  ...
  it('should not include apiVersion when AZURE_CODE_API_VERSION is not set', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
  ...
  it('should not include apiVersion when AZURE_CODE_API_VERSION is an empty string', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('AZURE_CODE_API_VERSION', '');
  ...
  it('should pass apiVersion for Vertex AI when AZURE_CODE_API_VERSION is set', async () => {
    const mockConfig = {
      getModel: vi.fn().mockReturnValue('gemini-pro'),
      getProxy: vi.fn().mockReturnValue(undefined),
      getUsageStatisticsEnabled: () => false,
    ...
    vi.mocked(GoogleGenAI).mockImplementation(() => mockGenerator as never);
    vi.stubEnv('AZURE_CODE_API_VERSION', 'v1alpha');
...
describe('createContentGeneratorConfig', () => {
  ...
  it('should configure for Gemini using AZURE_API_KEY when set', async () => {
    vi.stubEnv('AZURE_API_KEY', 'env-gemini-key');
  ...
  it('should not configure for Gemini if AZURE_API_KEY is empty', async () => {
  ...
  it('should not configure for Gemini if AZURE_API_KEY is not set and storage is empty', async () => {
    vi.stubEnv('AZURE_API_KEY', '');
  ...
  it('should configure for Vertex AI using AZURE_API_KEY when set', async () => {
    vi.stubEnv('AZURE_API_KEY', 'env-google-key');
    const config = await createContentGeneratorConfig(
      mockConfig,
  ...
  it('should configure for GATEWAY using dummy placeholder if AZURE_API_KEY is set', async () => {
    vi.stubEnv('AZURE_API_KEY', 'env-gemini-key');
    const config = await createContentGeneratorConfig(
      mockConfig,
      AuthType.GATEWAY,
  ...
  it('should configure for GATEWAY using dummy placeholder if AZURE_API_KEY is not set', async () => {
    vi.stubEnv('AZURE_API_KEY', '');
    vi.mocked(loadApiKey).mockResolvedValue(null);
    const config = await createContentGeneratorConfig(
      mockConfig,
