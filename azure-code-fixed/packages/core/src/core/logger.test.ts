...
import type { Content } from './azureApiClient.js';
...
const TEST_AZURE_DIR = path.join(
  os.homedir(),
  AZURE_DIR,
  TMP_DIR_NAME,
  PROJECT_SLUG,
...
const TEST_LOG_FILE_PATH = path.join(TEST_AZURE_DIR, LOG_FILE_NAME);
...
async function cleanupLogAndCheckpointFiles() {
  try {
    await fs.rm(TEST_AZURE_DIR, { recursive: true, force: true });
  } catch {
...
describe('Logger', () => {
  ...
  beforeEach(async () => {
    ...
    await fs.mkdir(TEST_AZURE_DIR, { recursive: true });
  ...
  describe('initialize', () => {
    ...
    it('should handle invalid JSON in log file by backing it up and starting fresh', async () => {
      ...
      const logContent = await readLogFile();
      expect(logContent).toEqual([]);
      const dirContents = await fs.readdir(TEST_AZURE_DIR);
      expect(
        dirContents.some(
          (f) =>
