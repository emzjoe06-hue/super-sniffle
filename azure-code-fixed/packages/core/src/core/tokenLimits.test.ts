...
describe('tokenLimit', () => {
  it('should return the correct token limit for default models', () => {
    expect(tokenLimit(DEFAULT_AZURE_MODEL)).toBe(1_048_576);
    expect(tokenLimit(DEFAULT_AZURE_FLASH_MODEL)).toBe(1_048_576);
    expect(tokenLimit(DEFAULT_AZURE_FLASH_LITE_MODEL)).toBe(1_048_576);
  });
