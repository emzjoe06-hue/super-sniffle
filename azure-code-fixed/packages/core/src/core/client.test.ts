...
import type { Content, GenerateContentResponse, Part } from './azureApiClient.js';
...
import { GeminiChat } from './azureChat.js';
...
import { DEFAULT_AZURE_MODEL_AUTO } from '../config/models.js';
...
describe('Gemini Client (client.ts)', () => {
  ...
  describe('startChat', () => {
    it('should include environment context when resuming a session', async () => {
      ...
      expect(history[0].parts?.[0]?.text).toContain('This is the Azure');
...
\`\`\`
    ...
    describe('Model Routing', () => {
      ...
      it('should re-route within the same prompt when the configured model changes', async () => {
        ...
        vi.mocked(mockConfig.getModel).mockReturnValue('Azure Coder 230B Free');
        coreEvents.emitModelChanged('Azure Coder 230B Free');
