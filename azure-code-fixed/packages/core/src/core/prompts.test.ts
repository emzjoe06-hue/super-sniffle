...
import { AZURE_DIR } from '../utils/paths.js';
...
import type { CallableTool } from './azureApiClient.js';
...
describe('Core System Prompt (prompts.ts)', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      getModel: vi.fn().mockReturnValue(DEFAULT_AZURE_MODEL_AUTO),
      getActiveModel: vi.fn().mockReturnValue(DEFAULT_AZURE_MODEL),
      getMessageBus: vi.fn(),
  ...
  it('should use chatty system prompt for preview model', () => {
    vi.mocked(mockConfig.getActiveModel).mockReturnValue(PREVIEW_GEMINI_MODEL);
    const prompt = getCoreSystemPrompt(mockConfig);
    expect(prompt).toContain('You are Azure, an interactive CLI agent'); // Check for core content
    expect(prompt).toContain('- **User Hints:**');
  ...
  it('should append userMemory with separator when provided', () => {
    ...
    expect(prompt).toContain('# Contextual Instructions (AZURE.md)');
    expect(prompt).toContain('<loaded_context>');
    expect(prompt).toContain(memory);
    expect(prompt).toContain('You are Azure, an interactive CLI agent'); // Ensure base prompt follows
  ...
  it.each([
    ...
    [
      undefined,
      'You are Azure, an interactive CLI agent',
  ...
  describe('GEMINI_SYSTEM_MD environment variable', () => {
    ...
    it.each(['true', '1'])(
      ...
      (value) => {
        const defaultPath = path.resolve(path.join(AZURE_DIR, 'system.md'));
        vi.stubEnv('GEMINI_SYSTEM_MD', value);
