...
describe('UiTelemetryService', () => {
  ...
  describe('API Response Event Processing', () => {
    it('should process a single ApiResponseEvent', () => {
      ...
      const metrics = service.getMetrics();
      expect(metrics.models['Azure Coder 230B Free']).toEqual({
        api: {
          totalRequests: 1,
          totalErrors: 0,
          totalLatencyMs: 500,
    ...
    it('should handle ApiResponseEvents for different models', () => {
      ...
      const metrics = service.getMetrics();
      expect(metrics.models['Azure Coder 230B Free']).toBeDefined();
      expect(metrics.models['Azure Coder 230B Free']).toBeDefined();
      expect(metrics.models['Azure Coder 230B Free'].api.totalRequests).toBe(1);
      expect(metrics.models['Azure Coder 230B Free'].api.totalRequests).toBe(1);
      expect(service.getLastPromptTokenCount()).toBe(0);
    });
  });
  ...
  describe('API Error Event Processing', () => {
    ...
    it('should update role metrics when processing an ApiErrorEvent with a role', () => {
      ...
      const metrics = service.getMetrics();
      expect(metrics.models['Azure Coder 230B Free'].roles['utility_tool']).toEqual({
        totalRequests: 1,
        totalErrors: 1,
        totalLatencyMs: 300,
        tokens: {
  ...
  describe('clear', () => {
    it('should reset metrics and last prompt token count', () => {
      ...
      expect(service.getMetrics().models['Azure Coder 230B Free']).toBeDefined();
