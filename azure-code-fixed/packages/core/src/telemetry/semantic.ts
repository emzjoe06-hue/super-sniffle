...
function toPart(part: Part): Part {
  if (typeof part === 'string') {
    return { text: part as string };
  }
  return part;
}
...
function toContent(content: ContentUnion): Content | undefined {
  if (typeof content === 'string') {
    ...
    // 2. It's an array of parts (Part[])
    return {
      role: 'user' as const,
      parts: (content as Part[]).map(toPart),
    };
  } else if ('parts' in content && 'role' in content) {
    // 3. It's a Content object
    return content as Content;
  } else if (isPart(content as Part)) {
    // 4. It's a single Part object
    return {
      role: 'user' as const,
