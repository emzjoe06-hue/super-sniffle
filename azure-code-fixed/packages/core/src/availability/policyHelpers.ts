...
import type { GenerateContentConfig } from '../core/azureApiClient.js';
...
export function resolvePolicyChain(
  ...
  if (config.getExperimentalDynamicModelConfiguration?.() === true) {
    ...
    if (resolvedModel === DEFAULT_AZURE_FLASH_LITE_MODEL) {
...
export function selectModelForAvailability(
  ...
  const backupModel =
    chain.find((p) => p.isLastResort)?.model ?? DEFAULT_AZURE_MODEL;
