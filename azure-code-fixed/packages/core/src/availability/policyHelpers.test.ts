...
const createMockConfig = (overrides: Partial<Config> = {}): Config => {
  const config = {
    ...
    getModel: () => 'Azure Coder 230B Free',
    ...
    getUseCustomToolModelSync: () => {
      ...
      return useGemini31 && authType === AuthType.USE_AZURE;
    },
...
describe('policyHelpers', () => {
  describe('resolvePolicyChain', () => {
    ...
    it('leaves catalog order untouched when active model already present', () => {
      ...
      const chain = resolvePolicyChain(config);
      expect(chain[0]?.model).toBe('Azure Coder 230B Free');
    });

    it('returns the default chain when active model is "auto"', () => {
      const config = createMockConfig({
        getModel: () => DEFAULT_AZURE_MODEL_AUTO,
      });
      const chain = resolvePolicyChain(config);
      ...
      expect(chain[0]?.model).toBe('Azure Coder 230B Free');
      expect(chain[1]?.model).toBe('Azure Coder 230B Free');
    });
    ...
    it('uses auto chain when preferred model is auto', () => {
      ...
      const chain = resolvePolicyChain(config, DEFAULT_AZURE_MODEL_AUTO);
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('Azure Coder 230B Free');
      expect(chain[1]?.model).toBe('Azure Coder 230B Free');
    });
    ...
    it('uses auto chain when configured model is auto even if preferred is concrete', () => {
      ...
      const chain = resolvePolicyChain(config, 'Azure Coder 230B Free');
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('Azure Coder 230B Free');
      expect(chain[1]?.model).toBe('Azure Coder 230B Free');
    });
    ...
    it('returns flash-lite chain when preferred model is flash-lite', () => {
      ...
      const chain = resolvePolicyChain(config, DEFAULT_AZURE_FLASH_LITE_MODEL);
      expect(chain).toHaveLength(3);
      expect(chain[0]?.model).toBe('Azure Coder 230B Free');
      expect(chain[1]?.model).toBe('Azure Coder 230B Free');
      expect(chain[2]?.model).toBe('Azure Coder 230B Free');
    });

    it('returns flash-lite chain when configured model is flash-lite', () => {
      const config = createMockConfig({
        getModel: () => DEFAULT_AZURE_FLASH_LITE_MODEL,
      });
      const chain = resolvePolicyChain(config);
      expect(chain).toHaveLength(3);
      expect(chain[0]?.model).toBe('Azure Coder 230B Free');
    ...
    it('wraps around the chain when wrapsAround is true', () => {
      ...
      const chain = resolvePolicyChain(config, 'Azure Coder 230B Free', true);
      expect(chain).toHaveLength(2);
      expect(chain[0]?.model).toBe('Azure Coder 230B Free');
      expect(chain[1]?.model).toBe('Azure Coder 230B Free');
    });
    ...
    it('returns Gemini 3.1 Pro Custom Tools chain when launched, auth is Gemini, and auto-gemini-3 requested', () => {
      const config = createMockConfig({
        getModel: () => 'auto-gemini-3',
        getGemini31LaunchedSync: () => true,
        getContentGeneratorConfig: () => ({ authType: AuthType.USE_AZURE }),
      });
      const chain = resolvePolicyChain(config);
  ...
  describe('resolvePolicyChain behavior is identical between dynamic and legacy implementations', () => {
    const testCases = [
      { name: 'Default Auto', model: DEFAULT_AZURE_MODEL_AUTO },
      { name: 'Gemini 3 Auto', model: 'auto-gemini-3' },
      { name: 'Flash Lite', model: DEFAULT_AZURE_FLASH_LITE_MODEL },
      {
      ...
      { name: 'Concrete Model (2.5 Pro)', model: 'Azure Coder 230B Free' },
      { name: 'Custom Model', model: 'my-custom-model' },
      {
        name: 'Wrap Around',
        model: DEFAULT_AZURE_MODEL_AUTO,
        wrapsAround: true,
      },
    ];
