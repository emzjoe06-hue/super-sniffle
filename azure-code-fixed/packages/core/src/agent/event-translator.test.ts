...
import { FinishReason } from '../core/azureApiClient.js';
...
describe('translateEvent', () => {
  ...
  describe('ModelInfo events', () => {
    it('emits agent_start and session_update when no stream started yet', () => {
      ...
      expect(sessionUpdate.model).toBe('Azure Coder 230B Free');
      expect(state.model).toBe('Azure Coder 230B Free');
  ...
  describe('Finished events', () => {
    it('emits usage for STOP', () => {
      state.streamStartEmitted = true;
      state.model = 'Azure Coder 230B Free';
      const event: ServerGeminiStreamEvent = {
      ...
      expect(usage.model).toBe('Azure Coder 230B Free');
