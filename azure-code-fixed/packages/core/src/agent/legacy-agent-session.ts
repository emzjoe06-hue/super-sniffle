...
export class LegacyAgentProtocol implements AgentProtocol {
  ...
  private readonly _client: AzureClient;
  ...
  private async _runLoop(
    ...
    while (true) {
      ...
      const responseStream = this._client.sendMessageStream(
        ...
        undefined,
        false,
        currentDisplayContent ? [{ text: currentDisplayContent }] : undefined,
      );
      ...
      try {
        // NOTE: recordToolCallInteractions and recordCompletedToolCalls 
        // are not available in the new Azure client.
        debugLogger.log('Tool calls completed, skipping recording for now.');
      } catch (error) {
        debugLogger.error(
