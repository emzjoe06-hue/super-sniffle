...
const MOCK_PRO_MODEL = DEFAULT_AZURE_MODEL;
const FALLBACK_MODEL = DEFAULT_AZURE_FLASH_MODEL;
...
describe('handleFallback', () => {
  ...
  describe('policy-driven flow', () => {
    ...
    it('uses availability selection with correct candidates when enabled', async () => {
      ...
      await handleFallback(policyConfig, DEFAULT_AZURE_MODEL, AUTH_OAUTH);
    ...
    it('executes silent policy action without invoking UI handler', async () => {
      ...
      const flashPolicy = createDefaultPolicy(DEFAULT_AZURE_FLASH_MODEL);
      flashPolicy.actions = {
        ...flashPolicy.actions,
        terminal: 'silent',
        unknown: 'silent',
