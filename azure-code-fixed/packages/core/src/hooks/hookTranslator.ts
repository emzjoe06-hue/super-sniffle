...
import { DEFAULT_AZURE_FLASH_MODEL } from '../config/models.js';
import { getResponseText } from '../utils/partUtils.js';

/**
 * Decoupled LLM request format - stable across Azure versions
 */
...
/**
 * Decoupled LLM response format - stable across Azure versions
 */
...
/**
 * Decoupled tool configuration - stable across Azure versions
 */
...
export class HookTranslatorGenAIv1 extends HookTranslator {
  ...
  toHookLLMRequest(sdkRequest: GenerateContentParameters): LLMRequest {
    ...
    return {
      model: sdkRequest.model || DEFAULT_AZURE_FLASH_MODEL,
      messages,
      config: {
        temperature: config?.temperature,
        maxOutputTokens: config?.maxOutputTokens,
