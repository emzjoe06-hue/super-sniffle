...
import { partListUnionToString } from '../core/azureRequest.js';
...
describe('GlobTool', () => {
  ...
  describe('ignore file handling', () => {
    ...
    it('should respect .azureignore files by default', async () => {
      await fs.writeFile(
        path.join(tempRootDir, AZURE_IGNORE_FILE_NAME),
        'azure-ignored_test.txt',
      );
      await fs.writeFile(
        path.join(tempRootDir, 'azure-ignored_test.txt'),
        'content',
      );
      await fs.writeFile(path.join(tempRootDir, 'visible_test.txt'), 'content');
      ...
      expect(result.llmContent).toContain('Found 1 file(s)');
      expect(result.llmContent).toContain('visible_test.txt');
      expect(result.llmContent).not.toContain('azure-ignored_test.txt');
    }, 30000);
    ...
    it('should not respect .azureignore when respect_gemini_ignore is false', async () => {
      await fs.writeFile(
        path.join(tempRootDir, AZURE_IGNORE_FILE_NAME),
        'azure-ignored_test.txt',
      );
      ...
      const params: GlobToolParams = {
        pattern: 'azure-ignored_test.txt',
      ...
      expect(result.llmContent).toContain('Found 1 file(s)');
      expect(result.llmContent).toContain('azure-ignored_test.txt');
    }, 30000);
  });
});
