  ...
  describe('shouldConfirmExecute', () => {
    ...
    it('should return confirmation details when memory file is not allowlisted', async () => {
      ...
      if (result && result.type === 'edit') {
        const expectedPath = path.join('~', AZURE_DIR, 'AZURE.md');
        expect(result.title).toBe(`Confirm Memory Save: ${expectedPath}`);
        expect(result.fileName).toContain(
          path.join('mock', 'home', AZURE_DIR),
        );
        expect(result.fileName).toContain('AZURE.md');
        expect(result.fileDiff).toContain('Index: AZURE.md');
