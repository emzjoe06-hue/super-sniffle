...
describe('ReadManyFilesTool', () => {
  ...
  beforeEach(async () => {
    ...
    fs.writeFileSync(path.join(tempRootDir, AZURE_IGNORE_FILE_NAME), 'foo.*');
  ...
  describe('execute', () => {
    ...
    it('should return error if path is ignored by a .azureignore pattern', async () => {
      createFile('foo.bar', '');
      createFile('bar.ts', '');
      createFile('foo.quux', '');
...
`,
  ...
  describe('JIT context discovery', () => {
    ...
    it('should discover JIT context sequentially to avoid duplicate shared parent context', async () => {
      ...
      // Simulate two subdirectories sharing a parent AZURE.md.
