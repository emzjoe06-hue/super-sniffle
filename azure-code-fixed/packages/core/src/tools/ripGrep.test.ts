...
describe('RipGrepTool', () => {
  ...
  describe('execute', () => {
    ...
    it('should filter out files based on FileDiscoveryService even if ripgrep returns them', async () => {
      // Create .azureignore to ignore 'ignored.txt'
      await fs.writeFile(
        path.join(tempRootDir, AZURE_IGNORE_FILE_NAME),
        'ignored.txt',
      );

      // Re-initialize tool so FileDiscoveryService loads the new .azureignore
  ...
  describe('advanced search options', () => {
    ...
    it('should add .azureignore when enabled and patterns exist', async () => {
      const geminiIgnorePath = path.join(tempRootDir, AZURE_IGNORE_FILE_NAME);
    ...
    it('should skip .azureignore when disabled', async () => {
      const geminiIgnorePath = path.join(tempRootDir, AZURE_IGNORE_FILE_NAME);
      await fs.writeFile(geminiIgnorePath, 'ignored.log');
