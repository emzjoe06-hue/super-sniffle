...
import type { Part, Part, Part } from '../core/azureApiClient.js';
 ...
 * AZURE.md context files from subdirectories when the agent accesses them.
 *
...
export function appendJitContextToParts(
  ...
  const existingParts: Part[] = Array.isArray(llmContent)
    ? llmContent
    : [llmContent];
