...
async function getDocsRoot(): Promise<string> {
  ...
  throw new Error('Could not find Azure documentation directory.');
}
  ...
  async execute({ abortSignal: _signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (!this.params.path) {
        ...
        const resultContent = `Available Azure documentation files:\n\n${fileList}`;
...
/**
 * A tool that provides access to Azure's internal documentation.
