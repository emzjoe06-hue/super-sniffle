...
// Environment was removed - need to use proper enum value
...
> {
  ...
  async execute({ abortSignal: signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      // Note: Azure Kilo API doesn't support computerUse tool declarations.
      // We do plain text analysis only.
      ...
      return {
        llmContent: [{ text: `Visual Analysis Result:\n${responseText}` }],
