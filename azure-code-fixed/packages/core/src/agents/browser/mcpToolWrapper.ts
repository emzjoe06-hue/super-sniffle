...
import type { FunctionDeclaration } from '../../core/azureApiClient.js';
...
> {
  ...
  async execute({ abortSignal: signal }: ExecuteOptions): Promise<ToolResult> {
    try {
      ...
      if (result.isError) {
        return {
          llmContent: [{ text: `Error: ${processedContent}` }],
      ...
      return {
        llmContent: [{ text: processedContent || 'Tool executed successfully.' }],
...
function convertMcpToolToFunctionDeclaration(
  ...
  // We pass it directly as parameters
  return {
    name: mcpTool.name,
