...
export class A2AAuthProviderFactory {
  static async create(
    ...
    switch (authConfig.type) {
      ...
      case 'google-credentials' as any:
        // Not supported in Azure fork
        throw new Error('google-credentials auth provider not supported in Azure fork');

      default: {
        // eslint-disable-next-line @typescript-eslint/no-explicit-any
        const _exhaustive = authConfig as any;
        throw new Error(
  ...
  static validateAuthConfig(
    ...
    return {
      ...
      diff: {
        requiredSchemes,
        configuredType: authConfig.type as import('./types.js').A2AAuthProviderType,
