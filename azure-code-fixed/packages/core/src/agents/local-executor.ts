...
import { AzureChat, StreamEventType } from '../core/azureChat.js';
import {
  type Content,
  type Part,
...
import { DEFAULT_AZURE_MODEL, isAutoModel } from '../config/models.js';
...
export class LocalAgentExecutor<TOutput extends z.ZodTypeAny> {
  ...
  private get executionContext(): AgentLoopContext {
    return {
      ...
      azureClient: this.context.azureClient,
  ...
  private async tryCompressChat(
    ...
    const model = this.definition.modelConfig.model ?? DEFAULT_AZURE_MODEL;
  ...
  private async callModel(
    ...
    if (isAutoModel(requestedModel)) {
      ...
      try {
        ...
        modelToUse = DEFAULT_AZURE_MODEL;
      }
    } else {
  ...
  /** Initializes a `AzureChat` instance for the agent run. */
...
* This is the ONLY way to complete your mission. If you stop calling tools without calling this, you have failed.`;
   ...
   */
  private parseToolArguments(functionCall: { name: string; args: Record<string, unknown> }): {
