...
describe('AgentRegistry', () => {
  ...
  describe('initialize', () => {
    ...
    it('should use default model for codebase investigator for non-preview models', async () => {
      const previewConfig = makeMockedConfig({ model: DEFAULT_AZURE_MODEL });
      ...
      expect(investigatorDef?.modelConfig.model).toBe(DEFAULT_AZURE_MODEL);
      expect(
      ...
      const previewConfig = makeMockedConfig({
        ...
        agents: {
          overrides: {
            codebase_investigator: {
              enabled: true,
              modelConfig: { model: DEFAULT_AZURE_FLASH_LITE_MODEL },
            },
          },
        },
      });
