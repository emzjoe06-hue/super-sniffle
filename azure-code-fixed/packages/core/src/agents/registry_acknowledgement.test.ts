...
describe('AgentRegistry Acknowledgement', () => {
  ...
  beforeEach(async () => {
    ...
    // Override AZURE_CODE_HOME to point to the temp directory
    originalGeminiCliHome = process.env['AZURE_CODE_HOME'];
    process.env['AZURE_CODE_HOME'] = tempDir;
  ...
  afterEach(async () => {
    ...
    if (originalGeminiCliHome) {
      process.env['AZURE_CODE_HOME'] = originalGeminiCliHome;
    } else {
      delete process.env['AZURE_CODE_HOME'];
    }
