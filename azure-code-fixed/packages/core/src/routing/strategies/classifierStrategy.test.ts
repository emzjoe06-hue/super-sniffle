...
import type { Content } from '../core/azureApiClient.js';
...
describe('ClassifierStrategy', () => {
  ...
  beforeEach(() => {
    ...
    mockConfig = {
      ...
      getUseCustomToolModel: vi.fn().mockImplementation(async () => {
        ...
        return launched && authType === AuthType.USE_AZURE;
      }),
  ...
  it('should NOT return null if numerical routing is enabled but model is NOT Gemini 3', async () => {
    vi.mocked(mockConfig.getNumericalRoutingEnabled).mockResolvedValue(true);
    vi.mocked(mockConfig.getModel).mockReturnValue(DEFAULT_AZURE_MODEL_AUTO);
  ...
  it('should route to FLASH model for a simple task', async () => {
    ...
    expect(decision).toEqual({
      model: DEFAULT_AZURE_FLASH_MODEL,
      metadata: {
        source: 'Classifier',
  ...
  it('should respect requestedModel from context in resolveClassifierModel', async () => {
    const requestedModel = DEFAULT_AZURE_MODEL; // Pro model
    const mockApiResponse = {
    ...
    expect(decision?.model).toBe(DEFAULT_AZURE_FLASH_MODEL);
  });
  ...
  describe('Gemini 3.1 and Custom Tools Routing', () => {
    ...
    it('should route to PREVIEW_GEMINI_3_1_CUSTOM_TOOLS_MODEL when Gemini 3.1 is launched and auth is USE_AZURE', async () => {
      vi.mocked(mockConfig.getGemini31Launched).mockResolvedValue(true);
      vi.mocked(mockConfig.getModel).mockReturnValue(PREVIEW_GEMINI_MODEL_AUTO);
