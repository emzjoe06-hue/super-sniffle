...
import type { Part } from '../core/azureApiClient.js';
import { type AzureClient } from '../core/client.js';
...
export class ToolOutputDistillationService {
  ...
  async distill(
    ...
    if (this.isExemptFromDistillation(toolName)) {
      return { truncatedContent: typeof content === 'string' ? [{ text: content }] : content };
    }
  ...
  private calculateContentLength(content: string | Part[]): number {
    ...
    if (Array.isArray(content)) {
      return content.reduce((acc, part) => {
        if (typeof part === 'string') return acc + (part as string).length;
  ...
  private stringifyContent(content: string | Part[]): string {
  ...
  private truncateContentStructurally(
    ...
    return content.map((part) => {
      if (typeof part === 'string') {
        ...
        return { text: truncateProportionally(text, targetChars, TOOL_TRUNCATION_PREFIX) };
      }
...
${stringifiedContent.slice(0, maxPreviewLen)}...`;
      ...
      const summaryResponse = await this.azureClient.generateContent(
