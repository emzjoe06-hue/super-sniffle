...
import { AzureClient } from '../core/client.js';
...
import type { GenerateContentParameters } from '../core/azureApiClient.js';
...
// ── Stub types/functions for Google Code Assist features not used in Azure fork ──
...
export enum ExperimentFlags {
  ...
  MASKING_PROTECTION_THRESHOLD = 'masking_protection_threshold',
  MASKING_PRUNABLE_THRESHOLD = 'masking_prunable_threshold',
  MASKING_PROTECT_LATEST_TURN = 'masking_protect_latest_turn',
  ...
  PRO_MODEL_NO_ACCESS = 'pro_model_no_access',
  GEMINI_3_1_PRO_LAUNCHED = 'gemini_3_1_pro_launched',
  GEMINI_3_1_FLASH_LITE_LAUNCHED = 'gemini_3_1_flash_lite_launched',
  DEFAULT_REQUEST_TIMEOUT = 'default_request_timeout',
}

// Stub: Azure fork has no Code Assist server
// eslint-disable-next-line @typescript-eslint/no-unused-vars
function getCodeAssistServer(_config: unknown): undefined {
  return undefined;
}

// Stub: Azure fork has no remote experiments
// eslint-disable-next-line @typescript-eslint/no-unused-vars
async function getExperiments(_server: unknown): Promise<Experiments> {
  return { flags: {} };
}

// Stub: Azure fork has no admin controls endpoint
async function fetchAdminControls(
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  _server: unknown,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  _existingSettings: unknown,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  _enabled: boolean,
  // eslint-disable-next-line @typescript-eslint/no-unused-vars
  _onUpdate: (settings: AdminControlsSettings) => void,
): Promise<AdminControlsSettings> {
  return {} as AdminControlsSettings;
}
 ...
 * directly.
 */
export interface AzureCodeExtension {
  name: string;
  version: string;
  isActive: boolean;
  path: string;
...
// Admin controls settings type (stub - no remote admin in Azure)
...
export class MCPServerConfig {
  constructor(
    ...
    readonly extension?: AzureCodeExtension,
    // OAuth configuration
...
export class Config implements McpContext, AgentLoopContext {
  ...
  private _geminiClient!: AzureClient;
  ...
  > = new Map();
  private lastRetrievedQuota?: unknown;
  ...
  constructor(params: ConfigParameters) {
    ...
    this.embeddingModel =
      params.embeddingModel ?? DEFAULT_AZURE_EMBEDDING_MODEL;
    ...
    this.fileFiltering = {
      ...
      respectAzureIgnore:
        params.fileFiltering?.respectAzureIgnore ??
        DEFAULT_FILE_FILTERING_OPTIONS.respectAzureIgnore,
    ...
    this._geminiClient = new AzureClient(this);
  ...
  async refreshAuth(
    ...
    if (
      this.contentGeneratorConfig?.authType === AuthType.USE_AZURE &&
      authMethod !== AuthType.USE_AZURE
    ) {
    ...
    if (
      ...
      this.setModel(DEFAULT_AZURE_MODEL_AUTO);
    }
  ...
  getUserTier(): string | undefined {
    // userTier is not part of ContentGenerator in Azure fork
    return undefined;
  }
  ...
  getUserPaidTier(): string | undefined {
    return undefined;
  }
  ...
  private getPooledQuota(): {
    ...
    const proModel = isPreview ? PREVIEW_GEMINI_MODEL : DEFAULT_AZURE_MODEL;
  ...
  async refreshUserQuota(): Promise<unknown | undefined> {
    const codeAssistServer = getCodeAssistServer(this);
  ...
  getLastRetrievedQuota(): unknown | undefined {
    return this.lastRetrievedQuota;
  }
  ...
  /** @deprecated Use azureClient getter */
  getGeminiClient(): AzureClient {
    return this.azureClient;
  }

  /**
   * Updates the system instruction with the latest user memory.
   * Whenever the user memory (AZURE.md files) is updated.
   */
  updateSystemInstructionIfInitialized(): void {
    const azureClient = this.azureClient;
    if (azureClient?.isInitialized()) {
      azureClient.updateSystemInstruction();
    }
  }
  ...
  getFileFilteringRespectGeminiIgnore(): boolean {
    return this.fileFiltering.respectAzureIgnore;
  }
  ...
  getFileFilteringOptions(): FileFilteringOptions {
    return {
      ...
      respectAzureIgnore: this.fileFiltering.respectAzureIgnore,
  ...
  getExtensions(): AzureCodeExtension[] {
  ...
  async getUseCustomToolModel(): Promise<boolean> {
    ...
    const authType = this.contentGeneratorConfig?.authType;
    return useGemini3_1 && authType === AuthType.USE_AZURE;
  }
  ...
  private onAgentsRefreshed = async () => {
    ...
    const client = this.azureClient;
    if (client?.isInitialized()) {
      await client.setTools();
      client.updateSystemInstruction();
    } else {
      debugLogger.debug(
        '[Config] AzureClient not initialized; skipping live prompt/tool refresh.',
      );
    }
  };
...
export { DEFAULT_AZURE_FLASH_MODEL };

// Stub type - was previously in code_assist
