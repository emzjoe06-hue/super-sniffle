...
import { createMockSandboxConfig } from 'azure-code-test-utils';
...
vi.mock('../tools/memoryTool', () => ({
  ...
  getCurrentGeminiMdFilename: vi.fn(() => 'AZURE.md'), // Mock the original filename
  DEFAULT_CONTEXT_FILENAME: 'AZURE.md',
  AZURE_DIR: '.gemini',
}));
...
describe('Server Config (config.ts)', () => {
  const MODEL = DEFAULT_AZURE_MODEL;
  ...
  describe('initialize', () => {
    ...
    describe('getGemini31LaunchedSync', () => {
      it.each([AuthType.USE_AZURE, AuthType.USE_VERTEX_AI, AuthType.GATEWAY])(
  ...
  describe('refreshAuth', () => {
    it('should refresh auth and update config', async () => {
      const config = new Config(baseParams);
      const authType = AuthType.USE_AZURE;
...
describe('Config getHooks', () => {
  ...
  describe('setModel', () => {
    it('should allow setting a pro (any) model and reset availability', () => {
      ...
      const proModel = 'Azure Coder 230B Free';
    ...
    it('should call onModelChange when a new model is set and should persist', () => {
      ...
      config.setModel(DEFAULT_AZURE_MODEL, false);

      expect(onModelChange).toHaveBeenCalledWith(DEFAULT_AZURE_MODEL);
    });
    ...
    it('should NOT call onModelChange when a new model is temporary', () => {
      ...
      config.setModel(DEFAULT_AZURE_MODEL, true);
...
describe('Config Quota & Preview Model Access', () => {
  ...
  describe('refreshUserQuota', () => {
    ...
    it('should return undefined pooled quota for non-auto models', async () => {
      ...
      config.setModel('Azure Coder 230B Free');
      await config.refreshUserQuota();
...
describe('Config JIT Initialization', () => {
  ...
  beforeEach(() => {
    ...
    mockMemoryContextManager = {
      ...
      getUserProjectMemory: vi.fn().mockReturnValue(''),
      getLoadedPaths: vi.fn().mockReturnValue(new Set(['/path/to/AZURE.md'])),
  ...
  it('should initialize MemoryContextManager, load memory, and delegate to it when experimentalJitContext is enabled', async () => {
    ...
    expect(config.getGeminiMdFilePaths()).toEqual(['/path/to/AZURE.md']);
  });
...
describe('Model Persistence Bug Fix (#19864)', () => {
  ...
  it('should NOT reset preview model for CodeAssist auth when refreshUserQuota is not called (no projectId)', async () => {
    ...
    expect(config.getModel()).toBe(PREVIEW_GEMINI_3_1_MODEL);
    expect(config.getModel()).not.toBe(DEFAULT_AZURE_MODEL_AUTO);
  });

  it('should NOT reset preview model for USE_AZURE (hasAccessToPreviewModel is set to true)', async () => {
    const mockContentConfig = {
      authType: AuthType.USE_AZURE,
    } as Partial<ContentGeneratorConfig> as ContentGeneratorConfig;
    ...
    // For USE_AZURE, hasAccessToPreviewModel should be set to true
  ...
  it('should persist model when user selects it with persistMode=true', () => {
    ...
    const config = new Config({
      ...baseParams,
      model: DEFAULT_AZURE_MODEL_AUTO, // Initial model
      onModelChange,
    });
