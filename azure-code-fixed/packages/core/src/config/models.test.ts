...
describe('isPreviewModel', () => {
  ...
  it('should return false for non-preview models', () => {
    expect(isPreviewModel(DEFAULT_AZURE_MODEL)).toBe(false);
...
describe('isProModel', () => {
  it('should return true for models containing "pro"', () => {
    ...
    expect(isProModel('Azure Coder 230B Free')).toBe(true);
  ...
  it('should return false for models without "pro"', () => {
    ...
    expect(isProModel('Azure Coder 230B Free')).toBe(false);
...
describe('supportsModernFeatures', () => {
  ...
  it('should return false for older Gemini models', () => {
    expect(supportsModernFeatures('Azure Coder 230B Free')).toBe(false);
    expect(supportsModernFeatures('Azure Coder 230B Free')).toBe(false);
...
describe('isGemini3Model', () => {
  ...
  it('should return false for Gemini 2 models', () => {
    expect(isGemini3Model('Azure Coder 230B Free')).toBe(false);
    expect(isGemini3Model('Azure Coder 230B Free')).toBe(false);
    expect(isGemini3Model(DEFAULT_AZURE_MODEL_AUTO)).toBe(false);
  });
...
describe('getDisplayString', () => {
  ...
  it('should return Auto (Gemini 2.5) for default auto model', () => {
    expect(getDisplayString(DEFAULT_AZURE_MODEL_AUTO)).toBe(
      'Auto (Gemini 2.5)',
    );
  });
  ...
  it('should return the model name as is for other models', () => {
    ...
    expect(getDisplayString(DEFAULT_AZURE_FLASH_LITE_MODEL)).toBe(
      DEFAULT_AZURE_FLASH_LITE_MODEL,
    );
  });
});
...
describe('supportsMultimodalFunctionResponse', () => {
  ...
  it('should return false for gemini-2 models', () => {
    expect(supportsMultimodalFunctionResponse('Azure Coder 230B Free')).toBe(false);
    expect(supportsMultimodalFunctionResponse('Azure Coder 230B Free')).toBe(false);
  });
...
describe('resolveModel', () => {
  describe('delegation logic', () => {
    ...
    it('should return the Default Pro model when auto-gemini-2.5 is requested', () => {
      const model = resolveModel(DEFAULT_AZURE_MODEL_AUTO);
      expect(model).toBe(DEFAULT_AZURE_MODEL);
    });
    ...
    it('should return the Default Flash-Lite model when flash-lite is requested', () => {
      ...
      expect(model).toBe(DEFAULT_AZURE_FLASH_LITE_MODEL);
    });
    ...
    it('should return the requested model as-is for explicit specific models', () => {
      expect(resolveModel(DEFAULT_AZURE_MODEL)).toBe(DEFAULT_AZURE_MODEL);
      expect(resolveModel(DEFAULT_AZURE_FLASH_MODEL)).toBe(
        DEFAULT_AZURE_FLASH_MODEL,
      );
      expect(resolveModel(DEFAULT_AZURE_FLASH_LITE_MODEL)).toBe(
        DEFAULT_AZURE_FLASH_LITE_MODEL,
      );
    });
  ...
  describe('hasAccessToPreview logic', () => {
    ...
    it('should return default flash model when access to preview is false and preview flash model is requested', () => {
      ...
      ).toBe(DEFAULT_AZURE_FLASH_MODEL);
    });
    ...
    it('should return default flash lite model when access to preview is false and preview flash lite model is requested', () => {
      ...
      ).toBe(DEFAULT_AZURE_FLASH_LITE_MODEL);
    });
    ...
    it('should still return default model when access to preview is false and auto-gemini-2.5 is requested', () => {
      expect(
        resolveModel(DEFAULT_AZURE_MODEL_AUTO, false, false, false, false),
      ).toBe(DEFAULT_AZURE_MODEL);
    });
  });
});
...
describe('isGemini2Model', () => {
  it('should return true for Azure Coder 230B Free', () => {
    expect(isGemini2Model('Azure Coder 230B Free')).toBe(true);
  });
...
describe('isAutoModel', () => {
  ...
  it('should return true for "auto-gemini-2.5"', () => {
    expect(isAutoModel(DEFAULT_AZURE_MODEL_AUTO)).toBe(true);
  });

  it('should return false for concrete models', () => {
    expect(isAutoModel(DEFAULT_AZURE_MODEL)).toBe(false);
...
describe('resolveClassifierModel', () => {
  ...
  it('should return pro model when alias is pro', () => {
    expect(
      resolveClassifierModel(DEFAULT_AZURE_MODEL_AUTO, GEMINI_MODEL_ALIAS_PRO),
    ).toBe(DEFAULT_AZURE_MODEL);
    expect(
...
describe('isActiveModel', () => {
  it('should return true for valid models when useGemini3_1 is false', () => {
    expect(isActiveModel(DEFAULT_AZURE_MODEL)).toBe(true);
    expect(isActiveModel(PREVIEW_GEMINI_MODEL)).toBe(true);
    expect(isActiveModel(DEFAULT_AZURE_FLASH_MODEL)).toBe(true);
  });
  ...
  it('should return true for other valid models when useGemini3_1 is true', () => {
    expect(isActiveModel(DEFAULT_AZURE_MODEL, true)).toBe(true);
  });
