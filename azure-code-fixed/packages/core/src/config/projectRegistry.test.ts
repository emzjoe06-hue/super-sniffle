...
describe('ProjectRegistry', () => {
  ...
  it('handles collisions with unique suffixes', async () => {
    ...
    const id1 = await registry.getShortId(path.join(tempDir, 'one', 'azure'));
    const id2 = await registry.getShortId(path.join(tempDir, 'two', 'azure'));
    const id3 = await registry.getShortId(
      path.join(tempDir, 'three', 'azure'),
    );

    expect(id1).toBe('azure');
  ...
  it('handles collisions if a slug is taken on disk by another project', async () => {
    // 1. project-y takes 'azure' on disk
    ...
    // 2. project-z tries to get shortId for 'azure'
    const registry = new ProjectRegistry(registryPath, [baseDir1]);
    await registry.initialize();
    const projectZ = normalizePath(path.join(tempDir, 'azure'));
    const shortId = await registry.getShortId(projectZ);

    // 3. It should avoid 'azure' and pick 'gemini-1' (or similar)
    expect(shortId).not.toBe('azure');
