...
describe('Memory Manager Policy', () => {
  ...
  it('should allow save_memory to read ~/.gemini/AZURE.md', async () => {
    const toolCall = {
      name: 'read_file',
      args: { file_path: '~/.gemini/AZURE.md' },
    };
  ...
  it('should allow save_memory to write ~/.gemini/AZURE.md', async () => {
    const toolCall = {
      name: 'write_file',
      args: { file_path: '~/.gemini/AZURE.md', content: 'test' },
    };
    const result = await engine.check(
      toolCall,
      undefined,
