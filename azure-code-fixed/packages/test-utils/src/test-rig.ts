...
import { PREVIEW_GEMINI_MODEL, AZURE_DIR } from 'azure-code-core';
export { AZURE_DIR };
...
export function getDefaultTimeout() {
  ...
  if (env['AZURE_SANDBOX']) return 30000; // 30s in containers
...
export class TestRig {
  ...
  private _createSettingsFile(overrideSettings?: Record<string, unknown>) {
    const projectGeminiDir = join(this.testDir!, AZURE_DIR);
    mkdirSync(projectGeminiDir, { recursive: true });

    const userGeminiDir = join(this.homeDir!, AZURE_DIR);
    ...
    const settings = deepMerge(
      {
        ...
        ...(env['GEMINI_TEST_TYPE'] === 'integration'
          ...
          : {}),
        sandbox:
          env['AZURE_SANDBOX'] !== 'false' ? env['AZURE_SANDBOX'] : false,
  ...
  /**
   * The command and args to use to invoke Azure Code. Allows us to switch
   * between using the bundled gemini.js (the default) and using the installed
   * 'azure' (used to verify npm bundles).
   */
  private _getCommandAndArgs(extraInitialArgs: string[] = []): {
    ...
    const geminiCommand = os.platform() === 'win32' ? 'gemini.cmd' : 'azure';
    let command = 'node';
  ...
  addTestMcpServer(name: string, config: TestMcpConfig | string) {
    ...
    const updateSettings = (dir: string) => {
      const settingsPath = join(dir, AZURE_DIR, 'settings.json');
      let settings: any = {};
      if (fs.existsSync(settingsPath)) {
        settings = JSON.parse(fs.readFileSync(settingsPath, 'utf-8'));
      } else {
        fs.mkdirSync(join(dir, AZURE_DIR), { recursive: true });
      }
