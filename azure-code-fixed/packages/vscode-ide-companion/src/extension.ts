...
import {
  ...
  IDE_DEFINITIONS,
  type IdeInfo,
} from 'azure-code-core/src/ide/detect-ide.js';
  ...
  try {
    ...
    if (
      ...
      const selection = await vscode.window.showInformationMessage(
        `A new version (${latestVersion}) of the Azure Code Companion extension is available.`,
...
export async function activate(context: vscode.ExtensionContext) {
  logger = vscode.window.createOutputChannel('Azure Code IDE Companion');
  ...
  if (
    ...
    void vscode.window.showInformationMessage(
      'Azure Code Companion extension successfully installed.',
    );
  ...
  context.subscriptions.push(
    ...
    vscode.commands.registerCommand('gemini-cli.runAzureCode', async () => {
      const workspaceFolders = vscode.workspace.workspaceFolders;
      if (!workspaceFolders || workspaceFolders.length === 0) {
        vscode.window.showInformationMessage(
          'No folder open. Please open a folder to run Azure Code.',
        );
        return;
      }
      ...
      if (workspaceFolders.length === 1) {
        ...
        selectedFolder = await vscode.window.showWorkspaceFolderPick({
          placeHolder: 'Select a folder to run Azure Code in',
        });
      }

      if (selectedFolder) {
        const geminiCmd = 'azure';
        const terminal = vscode.window.createTerminal({
          name: `Azure Code (${selectedFolder.name})`,
