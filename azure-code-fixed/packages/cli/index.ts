...
async function getMemoryNodeArgs(): Promise<string[]> {
  ...
  try {
    ...
    // Respect AZURE_CODE_HOME environment variable, falling back to os.homedir()
    const baseDir =
      process.env['AZURE_CODE_HOME'] || join(os.homedir(), '.gemini');
...
async function run() {
  if (!process.env['AZURE_CODE_NO_RELAUNCH'] && !process.env['SANDBOX']) {
    ...
    const newEnv = { ...process.env, AZURE_CODE_NO_RELAUNCH: 'true' };
