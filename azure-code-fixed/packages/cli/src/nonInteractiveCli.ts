...
import type { Content, Part } from 'azure-code-core';
  ...
  return promptIdContext.run(prompt_id, async () => {
    ...
    try {
      ...
      while (true) {
        ...
        const responseStream = azureClient.sendMessageStream(
        ...
        for await (const event of responseStream) {
          ...
          if (event.type === AzureEventType.Content) {
            const isRaw =
          ...
          } else if (event.type === AzureEventType.ToolCallRequest) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === AzureEventType.LoopDetected) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
          ...
          } else if (event.type === AzureEventType.MaxSessionTurns) {
            if (streamFormatter) {
              streamFormatter.emitEvent({
                type: JsonStreamEventType.ERROR,
                timestamp: new Date().toISOString(),
          ...
          } else if (event.type === AzureEventType.Error) {
            throw event.value.error;
          } else if (event.type === AzureEventType.AgentExecutionStopped) {
            ...
            return;
          } else if (event.type === AzureEventType.AgentExecutionBlocked) {
        ...
        if (toolCallRequests.length > 0) {
          ...
          // Record tool calls with full metadata before sending responses to Azure
          try {
            const currentModel =
              azureClient.getCurrentSequenceModel() ?? config.getModel();
            azureClient
              .getChat()
