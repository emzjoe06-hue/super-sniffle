...
import { AzureAgent, Session } from './acpClient.js';
...
import { ApprovalMode } from 'azure-code-core/src/policy/types.js';
...
vi.mock(
  'azure-code-core',
  async (
    importOriginal: () => Promise<typeof import('azure-code-core')>,
  ) => {
...
describe('AzureAgent', () => {
  ...
  beforeEach(() => {
    mockConfig = {
      ...
      getAzureClient: vi.fn().mockReturnValue({
      ...
      getAzure31LaunchedSync: vi.fn().mockReturnValue(false),
    ...
    agent = new AzureAgent(mockConfig, mockSettings, mockArgv, mockConnection);
  });
  ...
  it('should create a new session', async () => {
    ...
    expect(mockConfig.getAzureClient).toHaveBeenCalled();
  ...
  it('should include preview models when user has access', async () => {
    ...
    mockConfig.getAzure31LaunchedSync = vi.fn().mockReturnValue(true);
  ...
  it('should include gemini-3.1-flash-lite when useAzure31FlashLite is true', async () => {
    mockConfig.getHasAccessToPreviewModel = vi.fn().mockReturnValue(true);
    mockConfig.getAzure31LaunchedSync = vi.fn().mockReturnValue(true);
    mockConfig.getAzure31FlashLiteLaunchedSync = vi.fn().mockReturnValue(true);
  ...
  it('should fail session creation if Azure API key is missing', async () => {
    (loadSettings as unknown as Mock).mockImplementation(() => ({
      merged: {
        security: { auth: { selectedType: AuthType.USE_AZURE } },
        mcpServers: {},
      },
      setValue: vi.fn(),
    }));
    ...
    await expect(
      ...
      message: 'Azure API key is missing or not configured.',
    });
  });
...
describe('Session', () => {
  ...
  beforeEach(() => {
    ...
    } as unknown as Mocked<AzureChat>;
    mockTool = {
      kind: 'read',
  ...
  it('should call updatePolicy when tool permission triggers always allow', async () => {
    ...
    const { updatePolicy } = await import('azure-code-core');
