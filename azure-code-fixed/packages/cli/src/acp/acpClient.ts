...
import type { Content, Part, FunctionCall } from 'azure-code-core';
import {
  SettingScope,
  loadSettings,
  type LoadedSettings,
...
export async function runAcpClient(
  ...
  const connection = new acp.AgentSideConnection(
    (connection) => new AzureAgent(config, settings, argv, connection),
    stream,
  );
...
export class AzureAgent {
  ...
  static generateCallId(name: string): string {
    return `${name}-${Date.now()}-${++AzureAgent.callIdCounter}`;
  }
  ...
  async initialize(
    ...
    const authMethods = [
      ...
      {
        id: AuthType.USE_AZURE,
        name: 'Azure API key',
        description: 'Use an API key with Azure Developer API',
        _meta: {
          'api-key': {
            provider: 'azure',
          },
  ...
  async newSession({
    ...
    const authType =
      loadedSettings.merged.security.auth.selectedType || AuthType.USE_AZURE;
    ...
    try {
      ...
      // Extra validation for Azure API key
      ...
      if (
        ...
        authErrorMessage = 'Azure API key is missing or not configured.';
      }
    } catch (e) {
      isAuthenticated = false;
    ...
    const chat = await azureClient.startChat();
  ...
  async loadSession({
    ...
    await azureClient.resumeChat(clientHistory, {
...
export class Session {
  ...
  private async runTool(
    ...
    const callId = fc.id ?? AzureAgent.generateCallId(fc.name || 'unknown');
    const args = fc.args ?? {};
  ...
  async #resolvePrompt(
    ...
    if (pathSpecsToRead.length > 0) {
      ...
      const callId = AzureAgent.generateCallId(readManyFilesTool.name);
...
function buildAvailableModels(
  ...
  if (shouldShowPreviewModels) {
    mainOptions.unshift({
      value: MODEL_FREE,
      title: getDisplayString(MODEL_FREE),
      description: useAzure31
        ? 'Let Azure decide the best model for the task: Azure Coder 230B Free'
        : 'Let Azure decide the best model for the task: Azure Coder 230B Free',
    });
  }
  ...
  if (shouldShowPreviewModels) {
    ...
    const previewOptions = [
      ...
      {
        value: MODEL_RAPID,
        title: getDisplayString(MODEL_RAPID),
      },
    ];
