...
export class ExploreExtensionsCommand implements Command {
  ...
  async execute(
    ...
    const extensionsUrl = 'https://azurecode.com/extensions/';
    return {
      name: this.name,
...
export class UpdateExtensionCommand implements Command {
  ...
  async execute(
    ...
    return {
      name: this.name,
      data: 'Headless extension updating requires internal UI dispatches. Please use `azure extensions update` directly in the terminal.',
    };
  }
}
