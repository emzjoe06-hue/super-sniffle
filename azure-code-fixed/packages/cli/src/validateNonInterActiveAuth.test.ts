...
describe('validateNonInterActiveAuth', () => {
  let originalEnvAzure CodeApiKey: string | undefined;
  ...
  beforeEach(() => {
    originalEnvAzure CodeApiKey = process.env['AZURE_API_KEY'];
    originalEnvVertexAi = process.env['AZURE_CODE_USE_VERTEXAI'];
    originalEnvGcp = process.env['AZURE_CODE_USE_GCA'];
    delete process.env['AZURE_API_KEY'];
    delete process.env['AZURE_CODE_USE_VERTEXAI'];
    delete process.env['AZURE_CODE_USE_GCA'];
  ...
  afterEach(() => {
    if (originalEnvAzure CodeApiKey !== undefined) {
      process.env['AZURE_API_KEY'] = originalEnvAzure CodeApiKey;
    } else {
      delete process.env['AZURE_API_KEY'];
    }
    if (originalEnvVertexAi !== undefined) {
      process.env['AZURE_CODE_USE_VERTEXAI'] = originalEnvVertexAi;
    } else {
      delete process.env['AZURE_CODE_USE_VERTEXAI'];
    }
    if (originalEnvGcp !== undefined) {
      process.env['AZURE_CODE_USE_GCA'] = originalEnvGcp;
    } else {
      delete process.env['AZURE_CODE_USE_GCA'];
    }
    vi.restoreAllMocks();
  ...
  it('uses LOGIN_WITH_GOOGLE if AZURE_CODE_USE_GCA is set', async () => {
    process.env['AZURE_CODE_USE_GCA'] = 'true';
    const nonInteractiveConfig = createLocalMockConfig({});
    await validateNonInteractiveAuth(
      undefined,
      undefined,
  ...
  it('uses USE_AZURE if AZURE_API_KEY is set', async () => {
    process.env['AZURE_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createLocalMockConfig({});
    await validateNonInteractiveAuth(
      undefined,
      undefined,
  ...
  it('uses USE_VERTEX_AI if AZURE_CODE_USE_VERTEXAI is true (with GOOGLE_CLOUD_PROJECT and GOOGLE_CLOUD_LOCATION)', async () => {
    process.env['AZURE_CODE_USE_VERTEXAI'] = 'true';
  ...
  it('uses USE_VERTEX_AI if AZURE_CODE_USE_VERTEXAI is true and AZURE_API_KEY is set', async () => {
    process.env['AZURE_CODE_USE_VERTEXAI'] = 'true';
    process.env['AZURE_API_KEY'] = 'vertex-api-key';
    const nonInteractiveConfig = createLocalMockConfig({});
    await validateNonInteractiveAuth(
      undefined,
      undefined,
  ...
  it('uses LOGIN_WITH_GOOGLE if AZURE_CODE_USE_GCA is set, even with other env vars', async () => {
    process.env['AZURE_CODE_USE_GCA'] = 'true';
    process.env['AZURE_API_KEY'] = 'fake-key';
    process.env['AZURE_CODE_USE_VERTEXAI'] = 'true';
    process.env['GOOGLE_CLOUD_PROJECT'] = 'test-project';
  ...
  it('uses USE_VERTEX_AI if both AZURE_API_KEY and AZURE_CODE_USE_VERTEXAI are set', async () => {
    process.env['AZURE_API_KEY'] = 'fake-key';
    process.env['AZURE_CODE_USE_VERTEXAI'] = 'true';
    process.env['GOOGLE_CLOUD_PROJECT'] = 'test-project';
    process.env['GOOGLE_CLOUD_LOCATION'] = 'us-central1';
  ...
  it('uses USE_AZURE if AZURE_CODE_USE_VERTEXAI is false, AZURE_API_KEY is set, and project/location are available', async () => {
    process.env['AZURE_CODE_USE_VERTEXAI'] = 'false';
    process.env['AZURE_API_KEY'] = 'fake-key';
    process.env['GOOGLE_CLOUD_PROJECT'] = 'test-project';
    process.env['GOOGLE_CLOUD_LOCATION'] = 'us-central1';
    const nonInteractiveConfig = createLocalMockConfig({});
  ...
  it('succeeds if effectiveAuthType matches enforcedType', async () => {
    mockSettings.merged.security.auth.enforcedType = AuthType.USE_AZURE;
    process.env['AZURE_API_KEY'] = 'fake-key';
    const nonInteractiveConfig = createLocalMockConfig({});
    await validateNonInteractiveAuth(
      undefined,
