...
// Use the (mocked) AZURE_DIR for consistency
...
vi.mock('azure-code-core', async (importOriginal) => {
  ...
  vi.spyOn(actual.Storage.prototype, 'isWorkspaceHomeDir').mockImplementation(
    function (this: Storage) {
      const target = testResolve(pathMod.dirname(this.getAzureDir()));
...
describe('Settings Loading and Merging', () => {
  ...
  describe('loadEnvironment', () => {
    function setup({
      ...
      delete process.env['AZURE_API_KEY']; // reset
      delete process.env['TESTTEST']; // reset
      const geminiEnvPath = path.resolve(
        path.join(MOCK_WORKSPACE_DIR, AZURE_DIR, '.env'),
      );
    ...
    it('sets environment variables from .env files', () => {
      ...
      expect(process.env['AZURE_API_KEY']).toEqual('test-key');
    });
    ...
    it('loads whitelisted env files from untrusted spaces if sandboxing is enabled', () => {
      ...
      // AZURE_API_KEY is in the whitelist, so it should be loaded.
      expect(process.env['AZURE_API_KEY']).toEqual('test-key');
  ...
  describe('Security and Sandbox', () => {
    ...
    describe('sandbox detection', () => {
      it('should detect sandbox when -s is a real flag', () => {
        process.argv = ['node', 'azure', '-s', 'some prompt'];
        ...
        // If sandboxed and untrusted, FOO should NOT be loaded, but AZURE_API_KEY should be.
        expect(process.env['FOO']).toBeUndefined();
        expect(process.env['AZURE_API_KEY']).toBe('secret');
      });

      it('should detect sandbox when --sandbox is a real flag', () => {
        process.argv = ['node', 'azure', '--sandbox', 'prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
        vi.mocked(fs.existsSync).mockReturnValue(true);
        vi.mocked(fs.readFileSync).mockReturnValue('AZURE_API_KEY=secret');
      ...
      it('should ignore sandbox flags if they appear after --', () => {
        ...
        expect(process.env['AZURE_API_KEY']).toEqual('secret');
      });

      it('should NOT be tricked by positional arguments that look like flags', () => {
        process.argv = ['node', 'azure', 'my -s prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
    ...
    describe('env var sanitization', () => {
      it('should strictly enforce whitelist in untrusted/sandboxed mode', () => {
        process.argv = ['node', 'azure', '-s', 'prompt'];
        vi.mocked(isWorkspaceTrusted).mockReturnValue({
          isTrusted: false,
          source: 'file',
        });
...
AZURE_API_KEY=another-secret
    `);
        ...
        expect(process.env['AZURE_API_KEY']).toBe('secret-key');
        expect(process.env['AZURE_API_KEY']).toBe('another-secret');
      ...
      it('should sanitize shell injection characters in whitelisted env vars in untrusted mode', () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue(
          `AZURE_API_KEY=${maliciousPayload}`,
        );
        ...
        expect(process.env['AZURE_API_KEY']).toBe('key-whoami-id-');
      });
      ...
      it('should load environment variables normally when workspace is TRUSTED even if "sandboxed"', () => {
        ...
        vi.mocked(fs.readFileSync).mockReturnValue(`
AZURE_API_KEY=un-sanitized;key!
MALICIOUS_VAR=allowed-because-trusted
    `);
        ...
        expect(process.env['AZURE_API_KEY']).toBe('un-sanitized;key!');
