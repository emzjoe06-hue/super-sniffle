...
export function getSystemSettingsPath(): string {
  ...
  if (platform() === 'darwin') {
    return '/Library/Application Support/AzureCli/settings.json';
...
function findEnvFile(startDir: string): string | null {
  ...
  while (true) {
    ...
    if (parentDir === currentDir || !parentDir) {
      ...
      const homeAzureEnvPath = path.join(homedir(), AZURE_DIR, '.env');
      if (fs.existsSync(homeAzureEnvPath)) {
        return homeAzureEnvPath;
      }
...
export function loadEnvironment(
  ...
  if (envFilePath) {
    ...
    try {
      ...
      const isProjectEnvFile = !envFilePath.includes(AZURE_DIR);
