...
function getSandboxCommand(
  ...
  const environmentConfiguredSandbox =
    process.env['AZURE_SANDBOX']?.toLowerCase().trim() ?? '';
  sandbox =
  ...
  if (typeof sandbox === 'string' && sandbox) {
    ...
    if (sandbox !== 'windows-native' && !commandExists.sync(sandbox)) {
      throw new FatalSandboxError(
        `Missing sandbox command '${sandbox}' (from AZURE_SANDBOX)`,
      );
    }
  ...
  // AZURE_SANDBOX=lxc or sandbox: "lxc" in settings to enable it.
}
...
export async function loadSandboxConfig(
  ...
  const image =
    process.env['AZURE_SANDBOX_IMAGE'] ??
    process.env['AZURE_SANDBOX_IMAGE_DEFAULT'] ??
    customImage ??
