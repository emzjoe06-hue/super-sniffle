...
import { getPackageJson } from 'azure-code-core';
...
describe('loadSandboxConfig', () => {
  ...
  beforeEach(() => {
    ...
    delete process.env['AZURE_SANDBOX'];
  ...
  describe('with AZURE_SANDBOX environment variable', () => {
    it('should use docker if AZURE_SANDBOX=docker and it exists', async () => {
      process.env['AZURE_SANDBOX'] = 'docker';
    ...
    it('should throw if AZURE_SANDBOX is an invalid command', async () => {
      process.env['AZURE_SANDBOX'] = 'invalid-command';
    ...
    it('should throw if AZURE_SANDBOX command does not exist', async () => {
      process.env['AZURE_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'docker' (from AZURE_SANDBOX)",
      );
    });

    it('should use lxc if AZURE_SANDBOX=lxc and it exists', async () => {
      process.env['AZURE_SANDBOX'] = 'lxc';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
      expect(config).toEqual({
        enabled: true,
    ...
    it('should throw if AZURE_SANDBOX=lxc but lxc command does not exist', async () => {
      process.env['AZURE_SANDBOX'] = 'lxc';
      mockedCommandExistsSync.mockReturnValue(false);
      await expect(loadSandboxConfig({}, {})).rejects.toThrow(
        "Missing sandbox command 'lxc' (from AZURE_SANDBOX)",
      );
    });
  });
  ...
  describe('with sandbox: true', () => {
    ...
    it('should throw if sandbox: true but no command is found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: true })).rejects.toThrow(
        'AZURE_SANDBOX is true but failed to determine command for sandbox; ' +
          'install docker or podman or specify command in AZURE_SANDBOX',
      );
    });
  });
  ...
  describe("with sandbox: 'command'", () => {
    ...
    it('should throw if the specified command does not exist', async () => {
      ...
      await expect(
        ...
        "Missing sandbox command 'podman' (from AZURE_SANDBOX)",
      );
    });
  ...
  describe('image configuration', () => {
    it('should use image from AZURE_SANDBOX_IMAGE env var if set', async () => {
      process.env['AZURE_SANDBOX_IMAGE'] = 'env/image';
      process.env['AZURE_SANDBOX'] = 'docker';
      mockedCommandExistsSync.mockReturnValue(true);
      const config = await loadSandboxConfig({}, {});
      expect(config).toEqual({
  ...
  describe('with sandbox: runsc (gVisor)', () => {
    ...
    it('should use runsc via AZURE_SANDBOX environment variable', async () => {
      process.env['AZURE_SANDBOX'] = 'runsc';
      const config = await loadSandboxConfig({}, {});
    ...
    it('should prioritize AZURE_SANDBOX over CLI and settings', async () => {
      process.env['AZURE_SANDBOX'] = 'runsc';
      const config = await loadSandboxConfig(
    ...
    it('should throw if runsc binary not found', async () => {
      ...
      await expect(loadSandboxConfig({}, { sandbox: 'runsc' })).rejects.toThrow(
        "Missing sandbox command 'runsc' (from AZURE_SANDBOX)",
      );
    });
