...
import * as ServerConfig from 'azure-code-core';
...
const originalAzureModel = process.env['GEMINI_MODEL'];
...
afterEach(() => {
  ...
  if (originalAzureModel !== undefined) {
    process.env['GEMINI_MODEL'] = originalAzureModel;
  } else {
    delete process.env['GEMINI_MODEL'];
  }
  Object.defineProperty(process.stdout, 'isTTY', {
    ...
    it.each([
      {
        ...
        argv: ['node', 'script.js', 'Hi Azure'],
        expectedQuery: 'Hi Azure',
      ...
      },
      {
        ...
        expectedQuery: '@path ./file.md',
        expectedModel: 'Azure Coder 230B Free',
        debug: false,
      },
      {
        description:
...
describe('loadCliConfig', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('AZURE_API_KEY', 'test-api-key');
  ...
  it('should use default fileFilter options when unconfigured', async () => {
    ...
    expect(config.getFileFilteringRespectAzureIgnore()).toBe(
    ...
    vi.spyOn(ExtensionManager.prototype, 'getExtensions').mockReturnValue([
      {
        ...
        id: 'ext1-id',
        version: '1.0.0',
        contextFiles: ['/path/to/ext1/AZURE.md'],
        isActive: true,
      },
      {
        path: '/path/to/ext2',
...
describe('loadCliConfig model selection', () => {
  ...
  it('selects a model from settings.json if provided', async () => {
    ...
    expect(config.getModel()).toBe('Azure Coder 230B Free');
  });
  ...
  it('always prefers model from argv', async () => {
    process.argv = ['node', 'script.js', '--model', 'Azure Coder 230B Free-preview'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings({
        model: {
    ...
    expect(config.getModel()).toBe('Azure Coder 230B Free-preview');
  });
...
describe('loadCliConfig interactive', () => {
  ...
  it('should be interactive if positional prompt words are provided with other flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'Azure Coder 230B Free', 'Hello'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
      'test-session',
  ...
  it('should handle multiple positional words with flags', async () => {
    ...
    expect(argv.model).toBe('Azure Coder 230B Free');
  });
  ...
  it('should be interactive if no positional prompt words are provided with flags', async () => {
    process.stdin.isTTY = true;
    process.argv = ['node', 'script.js', '--model', 'Azure Coder 230B Free'];
    const argv = await parseArguments(createTestMergedSettings());
    const config = await loadCliConfig(
      createTestMergedSettings(),
      'test-session',
...
describe('loadCliConfig fileFiltering', () => {
  ...
  const testCases: Array<{
    ...
    {
      property: 'respectAzureIgnore',
      getter: (c) => c.getFileFilteringRespectAzureIgnore(),
      value: true,
    },
    {
      property: 'respectAzureIgnore',
  ...
  describe('extension plan settings', () => {
    ...
    it('should use plan directory from active extension when user has not specified one', async () => {
      ...
      vi.spyOn(ExtensionManager.prototype, 'getExtensions').mockReturnValue([
        ...
        } as unknown as AzureCodeExtension,
      ]);
