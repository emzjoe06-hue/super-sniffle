...
import {
  setAzureMdFilename as setServerAzureMdFilename,
  getCurrentAzureMdFilename,
  ApprovalMode,
...
import type { ExtensionEvents } from 'azure-code-core/src/utils/extensionLoader.js';
...
export async function parseArguments(
  ...
  const yargsInstance = yargs(rawArgv)
    ...
    .scriptName('azure')
    .usage(
      'Usage: gemini [options] [command]\n\nAzure - Defaults to interactive mode. Use -p/--prompt for non-interactive (headless) mode.',
    )
    .option('isCommand', {
      type: 'boolean',
      hidden: true,
  ...
  yargsInstance
    .command('$0 [query..]', 'Launch Azure', (yargsInstance) =>
      yargsInstance
        .positional('query', {
          description:
        ...
        .option('worktree', {
          ...
          skipValidation: true,
          description:
            'Start Azure in a new git worktree. If no name is provided, one is generated automatically.',
        ...
        .option('allowed-tools', {
          ...
          nargs: 1,
          description:
            '[DEPRECATED: Use Policy Engine instead See https://azurecode.com/docs/core/policy-engine] Tools that are allowed to run without confirmation',
          coerce: coerceCommaSeparated,
        })
        .option('extensions', {
          alias: 'e',
...
export async function loadCliConfig(
  ...
  if (argv.sandbox) {
    process.env['AZURE_SANDBOX'] = 'true';
  }
  ...
  // directly to the Config constructor in core, and have core handle setAzureMdFilename.
  // However, loadHierarchicalAzureMemory is called *before* createServerConfig.
  if (settings.context?.fileName) {
    setServerAzureMdFilename(settings.context.fileName);
  } else {
    // Reset to default if not provided in settings.
    setServerAzureMdFilename(getCurrentAzureMdFilename());
  }
  ...
  // so Azure has context of all open folders, not just the cwd.
  ...
  if (!experimentalJitContext) {
    // Call the (now wrapper) loadHierarchicalAzureMemory which calls the server's version
  ...
  const defaultModel = MODEL_FREE;
  const specifiedModel =
    argv.model || process.env['GEMINI_MODEL'] || settings.model?.name;

  const resolvedModel =
    specifiedModel === MODEL_FREE
      ? defaultModel
  ...
  return new Config({
    ...
    sessionId,
    clientVersion: await getVersion(),
    embeddingModel: DEFAULT_AZURE_EMBEDDING_MODEL,
...
async function resolveWorktreeSettings(
  ...
  try {
    ...
    if (isAzureWorktree(toplevel, projectRoot)) {
