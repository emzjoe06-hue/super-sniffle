...
export class ExtensionManager extends ExtensionLoader {
  ...
  private loadedExtensions: AzureCodeExtension[] | undefined;
  private loadingPromise: Promise<AzureCodeExtension[]> | null = null;
  ...
  async installOrUpdateExtension(
    ...
    let extension: AzureCodeExtension | null;
    try {
...
Would you like to attempt to install via "git clone" instead?`,
  ...
  protected override async startExtension(extension: AzureCodeExtension) {
  ...
  protected override async stopExtension(extension: AzureCodeExtension) {
   ...
   */
  async loadExtensions(): Promise<AzureCodeExtension[]> {
    if (this.loadedExtensions) {
    ...
    this.loadingPromise = (async () => {
      try {
        ...
        const builtExtensions = builtExtensionsOrNull.filter(
          (ext): ext is AzureCodeExtension => ext !== null,
        );
  ...
  async loadExtension(
    extensionDir: string,
  ): Promise<AzureCodeExtension | null> {
    if (this.loadingPromise) {
  ...
  toOutputString(extension: AzureCodeExtension): string {
