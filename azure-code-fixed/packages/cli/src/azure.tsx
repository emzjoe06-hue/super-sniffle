...
export function getNodeMemoryArgs(isDebugMode: boolean): string[] {
  ...
  if (process.env['AZURE_CODE_NO_RELAUNCH']) {
    return [];
  }
...
export async function main() {
  ...
  if (
    ...
    coreEvents.emitFeedback(
      'warning',
      'Warning: --allowed-tools cli argument and tools.allowed in settings.json are deprecated and will be removed in 1.0: Migrate to Policy Engine: https://azurecode.com/docs/core/policy-engine/',
    );
  }
  ...
  if (
    ...
    coreEvents.emitFeedback(
      'warning',
      'Warning: tools.exclude in settings.json is deprecated and will be removed in 1.0. Migrate to Policy Engine: https://azurecode.com/docs/core/policy-engine/',
    );
  }
  ...
  if (
    ...
    if (
      ...
      process.env['AZURE_CODE_USE_COMPUTE_ADC'] === 'true'
    ) {
      settings.setValue(
        SettingScope.User,
  ...
  // to run Azure. It is now safe to perform expensive initialization that
  // may have side effects.
  {
    ...
    if (!input) {
      debugLogger.error(
        `No input provided via stdin. Input can be provided by piping data into azure or using the --prompt option.`,
      );
      await runExitCleanup();
      process.exit(ExitCodes.FATAL_INPUT_ERROR);
    }
