...
// Mock AzureRespondingSpinner to disable animations (avoiding 'act()' warnings) without triggering screen reader mode.
vi.mock('../ui/components/AzureRespondingSpinner.js', async () => {
  const React = await import('react');
  const { Text } = await import('ink');
  return {
    AzureSpinner: () => React.createElement(Text, null, '...'),
    AzureRespondingSpinner: ({
...
export class AppRig {
  ...
  async initialize() {
    ...
    await act(async () => {
      ...
      await this.config!.refreshAuth(AuthType.USE_AZURE);
    });
  }

  private setupEnvironment() {
    // Stub environment variables to avoid interference from developer's machine
    vi.stubEnv('AZURE_CODE_HOME', this.testDir);
    ...
    if (this.options.fakeResponsesPath) {
      ...
      if (!process.env['AZURE_API_KEY']) {
        throw new Error(
          'AZURE_API_KEY must be set in the environment for live model tests.',
        );
      }
    ...
    return createMockSettings({
      user: {
        ...
        settings: {
          security: {
            auth: {
              selectedType: AuthType.USE_AZURE,
              useExternal: true,
            },
            folderTrust: {
              enabled: true,
  ...
  private stubRefreshAuth() {
    ...
    gcConfig.refreshAuth = async (authMethod: AuthType) => {
      ...
      const newContentGeneratorConfig = {
        ...
        apiKey: process.env['AZURE_API_KEY'] || 'test-api-key',
      };
      ...
      const { BaseLlmClient } = await import('azure-code-core');
