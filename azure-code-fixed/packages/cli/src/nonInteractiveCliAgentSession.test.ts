...
import type { Part } from 'azure-code-core';
...
vi.mock('azure-code-core', async (importOriginal) => {
  const original =
    await importOriginal<typeof import('azure-code-core')>();
...
describe('runNonInteractive', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getAzureClient: vi.fn().mockReturnValue(mockAzureClient),
  ...
  async function* createStreamFromEvents(
    events: ServerAzureStreamEvent[],
  ): AsyncGenerator<ServerAzureStreamEvent> {
  ...
  it('should process input and write text output', async () => {
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Hello' },
      { type: AzureEventType.Content, value: ' World' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ];
    mockAzureClient.sendMessageStream.mockReturnValue(
    ...
    expect(mockAzureClient.sendMessageStream).toHaveBeenCalledWith(
      [{ text: 'Test input' }],
  ...
  it('should stream the specific stream started by send', async () => {
    const { LegacyAgentSession } = await import('azure-code-core');
    const streamSpy = vi.spyOn(LegacyAgentSession.prototype, 'stream');
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Hello again' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle a single tool call and respond', async () => {
    const toolCallEvent: ServerAzureStreamEvent = {
      type: AzureEventType.ToolCallRequest,
      value: {
        callId: 'tool-1',
        name: 'testTool',
        args: { arg1: 'value1' },
    ...
    const firstCallEvents: ServerAzureStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Final answer' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
    ...
    mockAzureClient.sendMessageStream
    ...
    expect(mockAzureClient.sendMessageStream).toHaveBeenCalledTimes(2);
    expect(mockSchedulerSchedule).toHaveBeenCalledWith(
      [expect.objectContaining({ name: 'testTool' })],
      expect.any(AbortSignal),
    );
    expect(mockAzureClient.sendMessageStream).toHaveBeenNthCalledWith(
      2,
      [{ text: 'Tool response' }],
      expect.any(AbortSignal),
      'prompt-id-2',
  ...
  it('should write a single newline between sequential text outputs from the model', async () => {
    ...
    const modelTurn1: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Use mock tool' },
      toolCallEvent,
    ];
    // Turn 2: Model outputs more text, then requests another tool call.
    const modelTurn2: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Use mock tool again' },
      toolCallEvent,
    ];
    // Turn 3: Model outputs a final answer.
    const modelTurn3: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Finished.' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle error during tool execution and should send error back to the model', async () => {
    ...
    const finalResponse: ServerAzureStreamEvent[] = [
      {
        type: AzureEventType.Content,
  ...
  it('should exit with error if sendMessageStream throws initially', async () => {
    ...
    mockAzureClient.sendMessageStream.mockImplementation(() => {
      throw apiError;
    });
  ...
  it('should preprocess @include commands before sending to the model', async () => {
    ...
    // Mock a simple stream response from the Azure client
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Summary complete.' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should process input and write JSON output with stats', async () => {
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Hello World' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should write JSON output with stats for tool-only commands (no text response)', async () => {
    ...
    const firstCallEvents: ServerAzureStreamEvent[] = [
      toolCallEvent,
      {
        type: AzureEventType.Finished,
    ...
    mockAzureClient.sendMessageStream
      .mockReturnValueOnce(
        createStreamFromEvents([
          { type: AzureEventType.Content, value: 'Let me check that...' },
          toolCallEvent,
          {
            type: AzureEventType.Finished,
            value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
  ...
  it('should execute a slash command that returns a prompt', async () => {
    ...
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Response from command' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should handle slash commands', async () => {
    ...
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Response to slash command' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should handle cancellation (Ctrl+C)', async () => {
    ...
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Thinking...' },
    ];
  ...
  it('should treat an unknown slash command as a regular prompt', async () => {
    ...
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Response to unknown' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
  ...
  it('should pass arguments to the slash command action', async () => {
    ...
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Acknowledged' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 1 } },
      },
  ...
  it('should allow a normally-excluded tool when --allowed-tools is set', async () => {
    ...
    const firstCallEvents: ServerAzureStreamEvent[] = [toolCallEvent];
    const secondCallEvents: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'file.txt' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
      },
  ...
  it('should resume chat when resumedSessionData is provided', async () => {
    const events: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Resumed' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
    ...
    expect(mockAzureClient.resumeChat).toHaveBeenCalledWith(
      expect.any(Array),
      resumedSessionData,
    );
  ...
  it.each([
    {
      name: 'loop detected',
      events: [
        { type: AzureEventType.LoopDetected },
      ] as ServerAzureStreamEvent[],
      input: 'Loop test',
    ...
    },
    {
      ...
      events: [
        { type: AzureEventType.MaxSessionTurns },
      ] as ServerAzureStreamEvent[],
    ...
    async ({ events, input, promptId }) => {
      ...
      const streamEvents: ServerAzureStreamEvent[] = [
        ...events,
        {
          type: AzureEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 0 } },
  ...
  it('should log error when tool recording fails', async () => {
    ...
    const events: ServerAzureStreamEvent[] = [
      toolCallEvent,
      { type: AzureEventType.Content, value: 'Done' },
      {
        type: AzureEventType.Finished,
        value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
      },
    ...
    mockAzureClient.getChat = vi.fn().mockReturnValue(mockChat);
    mockAzureClient.getCurrentSequenceModel = vi
      .fn()
      .mockReturnValue('model-1');

    // Mock debugLogger.error
    const { debugLogger } = await import('azure-code-core');
  ...
  it('should stop agent execution immediately when a tool call returns STOP_EXECUTION error', async () => {
    ...
    const firstCallEvents: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Executing tool...' },
      toolCallEvent,
    ];
    ...
    expect(mockAzureClient.sendMessageStream).toHaveBeenCalledTimes(1);
  ...
  it('should write JSON output when a tool call returns STOP_EXECUTION error', async () => {
    ...
    const firstCallEvents: ServerAzureStreamEvent[] = [
      { type: AzureEventType.Content, value: 'Partial content' },
      toolCallEvent,
    ];
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event', async () => {
      const events: ServerAzureStreamEvent[] = [
        {
          type: AzureEventType.AgentExecutionStopped,
    ...
    it('should handle AgentExecutionBlocked event', async () => {
      const allEvents: ServerAzureStreamEvent[] = [
        {
          type: AzureEventType.AgentExecutionBlocked,
  ...
  describe('Output Sanitization', () => {
    ...
    it('should sanitize ANSI output by default', async () => {
      const events: ServerAzureStreamEvent[] = [
        { type: AzureEventType.Content, value: ANSI_SEQUENCE },
        { type: AzureEventType.Content, value: ' ' },
        { type: AzureEventType.Content, value: OSC_HYPERLINK },
        {
          type: AzureEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 10 } },
        },
    ...
    it('should emit warning event for loop_detected in streaming JSON mode', async () => {
      ...
      const streamEvents: ServerAzureStreamEvent[] = [
        { type: AzureEventType.LoopDetected } as ServerAzureStreamEvent,
        { type: AzureEventType.Content, value: 'Continuing after loop' },
        {
          type: AzureEventType.Finished,
          value: { reason: undefined, usageMetadata: { totalTokenCount: 5 } },
        },
