...
to the Azure Code Policy Engine.
...
1.  Link this extension to your local Azure Code installation:

    ```bash
    gemini extensions link packages/cli/src/commands/extensions/examples/policies
    ```

2.  Restart your Azure Code session.
...
For security, Azure Code ignores any `allow` decisions or `yolo` mode
