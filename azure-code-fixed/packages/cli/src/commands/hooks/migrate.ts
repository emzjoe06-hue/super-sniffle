...
import { debugLogger, getErrorMessage } from 'azure-code-core';
...
/**
 * Mapping from Claude Code event names to Azure event names
 */
const EVENT_MAPPING: Record<string, string> = {
  ...
  Stop: 'AfterAgent',
  SubAgentStop: 'AfterAgent', // Azure doesn't have sub-agents, map to AfterAgent
...
/**
 * Mapping from Claude Code tool names to Azure tool names
 */
...
/**
 * Transform a matcher regex to update tool names from Claude to Azure
 */
function transformMatcher(matcher: string | undefined): string | undefined {
  if (!matcher) return matcher;

  let transformed = matcher;
  for (const [claudeName, azureName] of Object.entries(TOOL_NAME_MAPPING)) {
...
/**
 * Migrate a Claude Code hook configuration to Azure format
 */
function migrateClaudeHook(claudeHook: unknown): unknown {
  ...
  if ('command' in hook) {
    ...
    // Replace CLAUDE_PROJECT_DIR with AZURE_PROJECT_DIR in command
  ...
  // Map timeout field (Claude uses seconds, Azure uses seconds)
  // eslint-disable-next-line no-restricted-syntax
...
/**
 * Migrate Claude Code hooks configuration to Azure format
 */
function migrateClaudeHooks(claudeConfig: unknown): Record<string, unknown> {
  ...
  const azureHooks: Record<string, unknown> = {};
  ...
  for (const [eventName, eventConfig] of Object.entries(hooksSection)) {
    // Map event name
    const azureEventName = EVENT_MAPPING[eventName] || eventName;
    ...
    azureHooks[azureEventName] = migratedDefinitions;
  }
...
export async function handleMigrateFromClaude() {
  ...
  try {
    ...
    debugLogger.log('✓ Hooks successfully migrated to .azure/settings.json');
    debugLogger.log(
      '\nMigration complete! Please review the migrated hooks in .azure/settings.json',
    );
  } catch (error) {
...
export const migrateCommand: CommandModule = {
  command: 'migrate',
  describe: 'Migrate hooks from Claude Code to Azure',
  builder: (yargs) =>
  ...
  handler: async (argv) => {
    ...
    if (args.fromClaude) {
      ...
      debugLogger.log(
        'Usage: azure hooks migrate --from-claude\n\nMigrate hooks from Claude Code to Azure format.',
      );
    }
    await exitCli();
  },
