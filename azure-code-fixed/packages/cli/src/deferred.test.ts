...
import { ExitCodes } from 'azure-code-core';
...
vi.mock('azure-code-core', async () => {
  const actual = await vi.importActual('azure-code-core');
  return {
    ...actual,
...
describe('deferred', () => {
  ...
  describe('runDeferredCommand', () => {
    ...
    it('should execute the deferred command if enabled', async () => {
      ...
      setDeferredCommand({
        handler: mockHandler,
        argv: { _: [], $0: 'azure' } as ArgumentsCamelCase,
        commandName: 'mcp',
      });
  ...
  describe('defer', () => {
    it('should wrap a command module and defer execution', async () => {
      ...
      const argv = { _: [], $0: 'azure' } as ArgumentsCamelCase;
