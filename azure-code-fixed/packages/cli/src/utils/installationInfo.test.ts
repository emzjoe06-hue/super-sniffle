...
import { isGitRepository, debugLogger } from 'azure-code-core';
...
describe('getInstallationInfo', () => {
  ...
  it('should detect global pnpm installation', () => {
    const pnpmPath = `/Users/test/.pnpm/global/5/node_modules/.pnpm/some-hash/node_modules/azure-code/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('pnpm add -g azure-code@latest');
  ...
  it('should detect global yarn installation', () => {
    const yarnPath = `/Users/test/.yarn/global/node_modules/azure-code/dist/index.js`;
    ...
    expect(info.updateCommand).toBe(
      'yarn global add azure-code@latest',
    );
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should detect global bun installation', () => {
    const bunPath = `/Users/test/.bun/install/global/node_modules/azure-code/dist/index.js`;
    ...
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('bun add -g azure-code@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should default to global npm installation for unrecognized paths', () => {
    ...
    expect(info.packageManager).toBe(PackageManager.NPM);
    expect(info.isGlobal).toBe(true);
    expect(info.updateCommand).toBe('npm install -g azure-code@latest');
    expect(info.updateMessage).toContain('Attempting to automatically update');
  ...
  it('should NOT detect Homebrew if gemini-cli is installed in brew but running from npm location', () => {
    ...
    const cliPath =
      '/usr/local/lib/node_modules/azure-code/dist/index.js';
    process.argv[1] = cliPath;
