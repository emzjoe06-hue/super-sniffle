  ...
  check: async (workspaceRoot: string, settings: Settings) => {
    ...
    try {
      ...
      if (workspaceRealPath === homeRealPath) {
        ...
        return 'Warning you are running Azure in your home directory.\nThis warning can be disabled in /settings';
      }
      return null;
    } catch {
...
const rootDirectoryCheck: WarningCheck = {
  ...
  check: async (workspaceRoot: string, _settings: Settings) => {
    try {
      ...
      const errorMessage =
        'Warning: You are running Azure in the root directory. Your entire folder structure will be used for context. It is strongly recommended to run in a project-specific directory.';
