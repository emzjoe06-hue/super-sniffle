...
describe('SessionSelector', () => {
  ...
  it('should list session with gemini message even without user message', async () => {
    const sessionIdAzure CodeOnly = randomUUID();
    ...
    const sessionAzure CodeOnly = {
      sessionId: sessionIdAzure CodeOnly,
      projectHash: 'test-hash',
      startTime: '2024-01-01T10:00:00.000Z',
      lastUpdated: '2024-01-01T10:30:00.000Z',
      messages: [
    ...
    await fs.writeFile(
      path.join(
        chatsDir,
        `${SESSION_FILE_PREFIX}2024-01-01T10-00-${sessionIdAzure CodeOnly.slice(0, 8)}.json`,
      ),
      JSON.stringify(sessionAzure CodeOnly, null, 2),
    );
    ...
    expect(sessions.length).toBe(1);
    expect(sessions[0].id).toBe(sessionIdAzure CodeOnly);
  });
