...
import type { Config, SandboxConfig } from 'azure-code-core';
import {
  coreEvents,
  debugLogger,
  FatalSandboxError,
  ...
  try {
    if (config.command === 'sandbox-exec') {
      ...
      if (!BUILTIN_SEATBELT_PROFILES.includes(profile)) {
        profileFile = path.join(AZURE_DIR, `sandbox-macos-${profile}.sb`);
      }
      ...
      // start and set up proxy if AZURE_SANDBOX_PROXY_COMMAND is set
      const proxyCommand = process.env['AZURE_SANDBOX_PROXY_COMMAND'];
      ...
      if (!gcPath.includes('gemini-cli/packages/')) {
        ...
        execSync(
          `cd ${gcRoot} && node scripts/build_sandbox.js -s ${buildArgs}`,
          {
            ...
            env: {
              ...process.env,
              AZURE_SANDBOX: command, // in case sandbox is enabled via flags (see config.ts under cli package)
            },
          },
        );
      }
    ...
    const userSettingsDirOnHost = path.join(userHomeDirOnHost, AZURE_DIR);
    ...
    // AZURE_SANDBOX_PROXY_COMMAND implies HTTPS_PROXY unless HTTP_PROXY is set
    const proxyCommand = process.env['AZURE_SANDBOX_PROXY_COMMAND'];
    ...
    // copy AZURE_API_KEY(s)
    if (process.env['AZURE_API_KEY']) {
      args.push('--env', `AZURE_API_KEY=${process.env['AZURE_API_KEY']}`);
    }
    if (process.env['AZURE_API_KEY']) {
      args.push('--env', `AZURE_API_KEY=${process.env['AZURE_API_KEY']}`);
    }
    ...
    // copy AZURE_CODE_USE_VERTEXAI
    if (process.env['AZURE_CODE_USE_VERTEXAI']) {
      args.push(
        '--env',
        `AZURE_CODE_USE_VERTEXAI=${process.env['AZURE_CODE_USE_VERTEXAI']}`,
      );
    }

    // copy AZURE_CODE_USE_GCA
    if (process.env['AZURE_CODE_USE_GCA']) {
      args.push(
        '--env',
        `AZURE_CODE_USE_GCA=${process.env['AZURE_CODE_USE_GCA']}`,
      );
    }
    ...
    if (
      ...
      const sandboxVenvPath = path.resolve(AZURE_DIR, 'sandbox.venv');
...
// image. The user creates and manages their own LXC container; Azure runs
