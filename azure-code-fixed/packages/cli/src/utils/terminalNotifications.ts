...
import { debugLogger, writeToStdout } from 'azure-code-core';
...
function sanitizeNotificationContent(
  ...
  return {
    ...
    body: body || 'Open Azure for details.',
  };
}
...
export function buildRunEventNotificationContent(
  ...
  if (event.type === 'attention') {
    return sanitizeNotificationContent({
      title: 'Azure needs your attention',
      subtitle: event.heading ?? 'Action required',
      body: event.detail ?? 'Open Azure to continue.',
    });
  }

  return sanitizeNotificationContent({
    title: 'Azure session complete',
