...
export const AppContainer = (props: AppContainerProps) => {
  ...
  const historyManager = useHistory({
    chatRecordingService: config.getAzureClient()?.getChatRecordingService(),
  });
  ...
  const [paidTier, setPaidTier] = useState<AzureUserTier | undefined>(
    undefined,
  );
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (result) {
        ...
        const azureClient = config.getAzureClient();
        if (additionalContext && azureClient) {
  ...
  const isAuthenticating =
    ...
    settings.merged.security.auth.selectedType !== AuthType.USE_AZURE;

  // Session browser and resume functionality
  const isAzureClientInitialized = config.getAzureClient()?.isInitialized();

  const { loadHistoryForResume, isResuming } = useSessionResume({
    config,
    historyManager,
    refreshStatic,
    isAzureClientInitialized,
    setQuittingMessages,
    resumedSessionData,
    isAuthenticating,
  });
    ...
    async (authType: AuthType | undefined, scope: LoadableSettingScope) => {
      if (authType) {
        ...
        if (
          ...
          writeToStdout(`
----------------------------------------------------------------
Configuring API key... Restarting Azure to continue.
----------------------------------------------------------------
          `);
          await relaunchApp();
        }
  ...
  const handleApiKeySubmit = useCallback(
    async (apiKey: string) => {
      try {
        ...
        await reloadApiKey();
        await config.refreshAuth(AuthType.USE_AZURE);
        setAuthState(AuthState.Authenticated);
      } catch (e) {
        onAuthError(
  ...
  useEffect(() => {
    if (
      ...
      // We skip validation for Azure API key here because it might be stored
      // in the keychain, which we can't check synchronously.
      // The useAuth hook handles validation for this case.
      if (settings.merged.security.auth.selectedType === AuthType.USE_AZURE) {
        return;
      }
  ...
  const performMemoryRefresh = useCallback(async () => {
    historyManager.addItem(
      {
        type: MessageType.INFO,
        text: 'Refreshing hierarchical memory (AZURE.md or other context files)...',
      },
      Date.now(),
    );
    try {
      ...
      if (config.isJitContextEnabled()) {
        ...
        fileCount = config.getAzureMdFileCount();
      } else {
  ...
  const {
    ...
    submitQuery,
    initError,
    pendingHistoryItems: pendingAzureHistoryItems,
    thought,
  ...
  const pendingHistoryItems = useMemo(
    () => [...pendingSlashCommandHistoryItems, ...pendingAzureHistoryItems],
    [pendingSlashCommandHistoryItems, pendingAzureHistoryItems],
  );
  ...
  useEffect(() => {
    ...
    // Note: We don't need to reset the window title on exit because Azure is already doing that elsewhere
  }, [
    streamingState,
    thought,
  ...
  const [geminiMdFileCount, setAzureMdFileCount] = useState<number>(
    config.getAzureMdFileCount(),
  );
  useEffect(() => {
    const handleMemoryChanged = (result: MemoryChangedPayload) => {
      setAzureMdFileCount(result.fileCount);
    };
