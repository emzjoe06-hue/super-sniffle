...
vi.mock('./hooks/useAzureStream.js');
...
import { useAzureStream } from './hooks/useAzureStream.js';
...
describe('AppContainer State Management', () => {
  ...
  const mockedUseAzureStream = useAzureStream as Mock;
  ...
  beforeEach(() => {
    ...
    mockedUseAzureStream.mockReturnValue(DEFAULT_GEMINI_STREAM_MOCK);
  ...
  describe('State Initialization', () => {
    it('sends a macOS notification when confirmation is pending and terminal is unfocused', async () => {
      ...
      mockedUseAzureStream.mockReturnValue({
    ...
    it('sends a macOS notification when a response completes while unfocused', async () => {
      ...
      mockedUseAzureStream.mockImplementation(() => ({
        ...DEFAULT_GEMINI_STREAM_MOCK,
      ...
      expect(
        ...
        expect.objectContaining({
          ...
          detail: 'Azure finished responding.',
        }),
      );
      expect(terminalNotificationsMocks.notifyViaTerminal).toHaveBeenCalled();
  ...
  describe('Session Resumption', () => {
    ...
    it('initializes chat recording service when config has it', async () => {
      ...
      vi.spyOn(configWithRecording, 'getAzureClient').mockReturnValue(
        mockAzureClient as unknown as ReturnType<Config['getAzureClient']>,
      );
  ...
  describe('Session Recording Integration', () => {
    it('provides chat recording service configuration', async () => {
      ...
      expect(configWithRecording.getAzureClient).toBeDefined();
      expect(mockAzureClient.getChatRecordingService).toBeDefined();
  ...
  describe('Session Resume Flow', () => {
    it('accepts resumed session data', async () => {
      ...
      vi.spyOn(configWithClient, 'getAzureClient').mockReturnValue(
        mockAzureClient as unknown as ReturnType<Config['getAzureClient']>,
      );
      ...
      expect(mockAzureClient.resumeChat).toBeDefined();
  ...
  describe('Terminal Title Update Feature', () => {
    ...
    it('should use legacy terminal title when dynamicWindowTitle is false', async () => {
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]0;${'Azure (workspace)'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
    ...
    it('should use CLI_TITLE environment variable when set', async () => {
      ...
      vi.stubEnv('CLI_TITLE', 'Custom Azure Title');
      ...
      expect(titleWrites).toHaveLength(1);
      expect(titleWrites[0][0]).toBe(
        `\x1b]0;${'✦  Working… (Custom Azure Title)'.padEnd(80, ' ')}\x07`,
      );
      unmount();
    });
  });
  ...
  describe('Keyboard Input Handling (CTRL+C / CTRL+D)', () => {
    ...
    describe('Background Shell Toggling (CTRL+B)', () => {
      ...
      it('should show and focus background shell on Ctrl+B if hidden', async () => {
        ...
        mockedUseAzureStream.mockReturnValue(geminiStreamMock);
  ...
  describe('Banner Text', () => {
    it('should render placeholder banner text for USE_AZURE auth type', async () => {
  ...
  describe('onCancelSubmit Behavior', () => {
    ...
    // Helper to extract arguments from the useAzureStream hook call
    // This isolates the positional argument dependency to a single location
    const extractUseAzureStreamArgs = (args: unknown[]) => ({
    ...
    it('preserves buffer when cancelling, even if empty (user is in control)', async () => {
      ...
      const { onCancelSubmit } = extractUseAzureStreamArgs(
        mockedUseAzureStream.mock.lastCall!,
      );
  ...
  describe('Permission Handling', () => {
    ...
    it.each([true, false])(
      ...
      async (allowed) => {
        ...
        const { submitQuery } = mockedUseAzureStream();
