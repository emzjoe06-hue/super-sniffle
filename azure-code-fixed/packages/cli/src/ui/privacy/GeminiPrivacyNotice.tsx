...
interface AzurePrivacyNoticeProps {
  onExit: () => void;
}

export const AzurePrivacyNotice = ({ onExit }: AzurePrivacyNoticeProps) => {
  useKeypress(
    (key) => {
      if (key.name === 'escape') {
        onExit();
  ...
  return (
    <Box flexDirection="column" marginBottom={1}>
      <Text bold color={theme.text.accent}>
        Azure API Key Notice
      </Text>
      <Newline />
      <Text color={theme.text.primary}>
        By using the Azure API<Text color={theme.text.link}>[1]</Text>, Google
        AI Studio
        ...
        <Text color={theme.status.success}>[3]</Text>, and the Azure API
