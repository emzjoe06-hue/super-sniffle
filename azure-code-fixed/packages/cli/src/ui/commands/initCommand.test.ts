...
import type { SubmitPromptActionReturn } from 'azure-code-core';
...
describe('initCommand', () => {
  ...
  const geminiMdPath = path.join(targetDir, 'AZURE.md');
  ...
  it('should inform the user if AZURE.md already exists', async () => {
    ...
    expect(result).toEqual({
      ...
      messageType: 'info',
      content:
        'A AZURE.md file already exists in this directory. No changes were made.',
    });
    // Assert: Ensure no file was written
    expect(fs.writeFileSync).not.toHaveBeenCalled();
  });

  it('should create AZURE.md and submit a prompt if it does not exist', async () => {
    ...
    expect(mockContext.ui.addItem).toHaveBeenCalledWith(
      {
        type: 'info',
        text: 'Empty AZURE.md created. Now analyzing the project to populate it.',
      },
      expect.any(Number),
    );
    ...
    expect(result.content).toContain(
      'You are an AI agent that brings the power of Azure',
    );
  });
