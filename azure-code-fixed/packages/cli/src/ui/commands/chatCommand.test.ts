...
import type { Content } from 'azure-code-core';
import { AuthType, type AzureClient } from 'azure-code-core';
  ...
  describe('resume subcommand', () => {
    ...
    it('should block resuming a conversation with mismatched authType', async () => {
      ...
      expect(result).toEqual({
        type: 'message',
        messageType: 'error',
        content: `Cannot resume chat. It was saved with a different authentication method (${AuthType.USE_AZURE}) than the current one (${AuthType.LOGIN_WITH_GOOGLE}).`,
      });
    });
