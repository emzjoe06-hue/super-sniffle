...
import type { MessageActionReturn } from 'azure-code-core';
import {
  DiscoveredMCPTool,
...
const authCommand: SlashCommand = {
  ...
  action: async (
    ...
    try {
      ...
      const { MCPOAuthProvider } = await import('azure-code-core');
      ...
      const azureClient = context.services.agentContext?.azureClient;
...
const reloadCommand: SlashCommand = {
  ...
  action: async (
    ...
    // Update the client with the new tools
    const azureClient = agentContext.azureClient;
    if (azureClient?.isInitialized()) {
      await azureClient.setTools();
    }
...
async function handleEnableDisable(
  ...
  if (agentContext.azureClient?.isInitialized())
    await agentContext.azureClient.setTools();
  context.ui.reloadCommands();
