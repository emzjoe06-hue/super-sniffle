  ...
  return {
    ...
    refreshMemory: vi.fn(async (config) => {
      if (config.isJitContextEnabled()) {
        ...
        const fileCount = config.getAzureMdFileCount() || 0;
        return {
          type: 'message',
          messageType: 'info',
...
describe('memoryCommand', () => {
  ...
  describe('/memory show', () => {
    ...
    let mockGetAzureMdFileCount: Mock;

    beforeEach(() => {
      showCommand = getSubCommand('show');

      mockGetUserMemory = vi.fn();
      mockGetAzureMdFileCount = vi.fn();
        ...
        services: {
          agentContext: {
            config: {
              ...
              getAzureMdFileCount: mockGetAzureMdFileCount,
    ...
    it('should display a message if memory is empty', async () => {
      ...
      mockGetAzureMdFileCount.mockReturnValue(0);
    ...
    it('should display the memory content and file count if it exists', async () => {
      ...
      mockGetAzureMdFileCount.mockReturnValue(1);
  ...
  describe('/memory reload', () => {
    ...
    let mockSetAzureMdFileCount: Mock;
    let mockSetAzureMdFilePaths: Mock;
    let mockContextManagerRefresh: Mock;

    beforeEach(() => {
      reloadCommand = getSubCommand('reload');
      mockSetUserMemory = vi.fn();
      mockSetAzureMdFileCount = vi.fn();
      mockSetAzureMdFilePaths = vi.fn();
      mockContextManagerRefresh = vi.fn().mockResolvedValue(undefined);

      const mockConfig = {
        setUserMemory: mockSetUserMemory,
        setAzureMdFileCount: mockSetAzureMdFileCount,
        setAzureMdFilePaths: mockSetAzureMdFilePaths,
        ...
        getAzureMdFileCount: vi.fn().mockReturnValue(0),
      };
    ...
    it('should use ContextManager.refresh when JIT is enabled', async () => {
      ...
      vi.mocked(config.getAzureMdFileCount).mockReturnValue(3);
    ...
    it('should display an error message if reloading fails', async () => {
      ...
      expect(mockSetAzureMdFileCount).not.toHaveBeenCalled();
      expect(mockSetAzureMdFilePaths).not.toHaveBeenCalled();
  ...
  describe('/memory list', () => {
    ...
    let mockGetAzure CodeMdfilePaths: Mock;

    beforeEach(() => {
      listCommand = getSubCommand('list');
      mockGetAzure CodeMdfilePaths = vi.fn();
      vi.mocked(listMemoryFiles).mockImplementation((config) => {
        const filePaths = config.getAzureMdFilePaths() || [];
        const fileCount = filePaths.length;
        let content;
        if (fileCount > 0) {
          content = `There are ${fileCount} AZURE.md file(s) in use:\n\n${filePaths.join('\n')}`;
        } else {
          content = 'No AZURE.md files in use.';
        }
        return {
          type: 'message',
          messageType: 'info',
      ...
      mockContext = createMockCommandContext({
        services: {
          agentContext: {
            config: {
              getAzureMdFilePaths: mockGetAzure CodeMdfilePaths,
            },
          },
        },
      });
    ...
    it('should display a message if no AZURE.md files are found', async () => {
      if (!listCommand.action) throw new Error('Command has no action');

      mockGetAzure CodeMdfilePaths.mockReturnValue([]);
    ...
    it('should display the file count and paths if they exist', async () => {
      ...
      const filePaths = ['/path/one/AZURE.md', '/path/two/AZURE.md'];
      mockGetAzure CodeMdfilePaths.mockReturnValue(filePaths);
      ...
      expect(mockContext.ui.addItem).toHaveBeenCalledWith(
        {
          type: MessageType.INFO,
          text: `There are 2 AZURE.md file(s) in use:\n\n${filePaths.join('\n')}`,
        },
        expect.any(Number),
      );
    });
