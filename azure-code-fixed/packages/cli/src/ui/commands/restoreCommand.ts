...
async function restoreAction(
  ...
  try {
    ...
    // 2. We trust that files valid according to this schema (written by useAzureStream) contain the full HistoryItem structure.
    ...
    for await (const action of actionStream) {
      if (action.type === 'message') {
        ...
        if (action.clientHistory) {
          agentContext!.azureClient?.setHistory(action.clientHistory);
        }
      }
    }
