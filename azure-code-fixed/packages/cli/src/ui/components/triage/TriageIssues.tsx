...
export const TriageIssues = ({
  ...
  const analyzeIssue = useCallback(
    async (issue: Issue): Promise<AnalysisResult> => {
      ...
      const prompt = `
I am triaging GitHub issues for the Azure project. I need to identify issues that should be closed because they are:
    ...
    <Box flexDirection="column">
      ...
      >
        {state.status === 'analyzing' ? (
          <Box>
            <Spinner type="dots" />
            <Text> Analyzing issue with Azure...</Text>
          </Box>
        ) : analysis ? (
          <>
