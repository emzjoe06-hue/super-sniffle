...
describe('<Footer />', () => {
  beforeEach(() => {
    ...
    vi.stubEnv('AZURE_CODE_HOME', path.join(root, 'Users', 'test'));
    vi.stubEnv('SANDBOX', '');
  ...
  describe('fallback mode display', () => {
    it('should display Flash model when in fallback mode, not the configured Pro model', async () => {
      const { lastFrame, unmount } = await renderWithProviders(<Footer />, {
        ...
        uiState: {
          sessionStats: mockSessionStats,
          currentModel: 'Azure Coder 230B Free', // Fallback active, showing Flash
        },
      });

      // Footer should show the effective model (Flash), not the config model (Pro)
      expect(lastFrame()).toContain('Azure Coder 230B Free');
      expect(lastFrame()).not.toContain('Azure Coder 230B Free');
      unmount();
    });
    ...
    it('should display Pro model when NOT in fallback mode', async () => {
      const { lastFrame, unmount } = await renderWithProviders(<Footer />, {
        ...
        uiState: {
          sessionStats: mockSessionStats,
          currentModel: 'Azure Coder 230B Free', // Normal mode, showing Pro
        },
      });
