  ...
  describe('for flash model failures', () => {
    it('should render "Keep trying" and "Stop" options', async () => {
      const { unmount } = await render(
        <ProQuotaDialog
          failedModel={DEFAULT_AZURE_FLASH_MODEL}
          fallbackModel={DEFAULT_AZURE_FLASH_MODEL}
          message="flash error"
  ...
  describe('for non-flash model failures', () => {
    describe('when it is a terminal quota error', () => {
      it('should render switch, upgrade, and stop options for LOGIN_WITH_GOOGLE', async () => {
        const { unmount } = await render(
          <ProQuotaDialog
            failedModel="Azure Coder 230B Free"
            fallbackModel="Azure Coder 230B Free"
        ...
        expect(RadioButtonSelect).toHaveBeenCalledWith(
          expect.objectContaining({
            items: [
              {
                label: 'Switch to Azure Coder 230B Free',
                value: 'retry_always',
                key: 'retry_always',
              },
              {
      ...
      it('should NOT render upgrade option for USE_AZURE', async () => {
        const { unmount } = await render(
          <ProQuotaDialog
            failedModel="Azure Coder 230B Free"
            fallbackModel="Azure Coder 230B Free"
