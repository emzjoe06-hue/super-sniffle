...
import type { Config, ModelSlashCommandEvent } from 'azure-code-core';
...
describe('<ModelDialog />', () => {
  ...
  const mockGetAzure31LaunchedSync = vi.fn();
  const mockGetAzure31FlashLiteLaunchedSync = vi.fn();
  ...
  interface MockConfig extends Partial<Config> {
    ...
    getIdeMode: () => boolean;
    getAzure31LaunchedSync: () => boolean;
    getAzure31FlashLiteLaunchedSync: () => boolean;
  ...
  const mockConfig: MockConfig = {
    ...
    getIdeMode: () => false,
    getAzure31LaunchedSync: mockGetAzure31LaunchedSync,
    getAzure31FlashLiteLaunchedSync: mockGetAzure31FlashLiteLaunchedSync,
  ...
  beforeEach(() => {
    vi.resetAllMocks();
    mockGetModel.mockReturnValue(DEFAULT_AZURE_MODEL);
    mockGetHasAccessToPreviewModel.mockReturnValue(false);
    mockGetAzure31LaunchedSync.mockReturnValue(false);
    mockGetAzure31FlashLiteLaunchedSync.mockReturnValue(false);
    mockGetProModelNoAccess.mockResolvedValue(false);
    mockGetProModelNoAccessSync.mockReturnValue(false);

    // Default implementation for getDisplayString
    mockGetDisplayString.mockImplementation((val: string) => {
      if (val === 'auto-gemini-2.5') return 'Auto (Azure 2.5)';
  ...
  it('renders the "manual" view initially for users with no pro access and filters Pro models with correct order', async () => {
    ...
    mockGetAzure31FlashLiteLaunchedSync.mockReturnValue(true);
    ...
    expect(output).not.toContain(DEFAULT_AZURE_MODEL);
    ...
    const flashIdx = output.indexOf(DEFAULT_AZURE_FLASH_MODEL);
    const flashLiteIdx = output.indexOf(DEFAULT_AZURE_FLASH_LITE_MODEL);
  ...
  it('switches to "manual" view when "Manual" is selected and uses getDisplayString for models', async () => {
    mockGetDisplayString.mockImplementation((val: string) => {
      if (val === DEFAULT_AZURE_MODEL) return 'Formatted Pro Model';
      if (val === DEFAULT_AZURE_FLASH_MODEL) return 'Formatted Flash Model';
      if (val === DEFAULT_AZURE_FLASH_LITE_MODEL)
  ...
  it('sets model and closes when a model is selected in "manual" view', async () => {
    ...
    // Now in manual view. Default selection is first item (DEFAULT_AZURE_MODEL)
    await act(async () => {
      stdin.write('\r');
    });
    await waitUntilReady();
    ...
    await waitFor(() => {
      expect(mockSetModel).toHaveBeenCalledWith(DEFAULT_AZURE_MODEL, true);
      expect(mockOnClose).toHaveBeenCalled();
    });
    unmount();
  });
  ...
  it('goes back to "main" view on escape in "manual" view', async () => {
    ...
    await waitFor(() => {
      expect(lastFrame()).toContain(DEFAULT_AZURE_MODEL);
    });
  ...
  it('shows the preferred manual model in the main view option using getDisplayString', async () => {
    mockGetModel.mockReturnValue(DEFAULT_AZURE_MODEL);
    mockGetDisplayString.mockImplementation((val: string) => {
      if (val === DEFAULT_AZURE_MODEL) return 'My Custom Model Display';
      if (val === 'auto-gemini-2.5') return 'Auto (Azure 2.5)';
      return val;
    });
    const { lastFrame, unmount } = await renderComponent();
  ...
  describe('Preview Models', () => {
    ...
    it('shows Azure 3 models in manual view when Azure 3.1 is NOT launched', async () => {
      mockGetAzure31LaunchedSync.mockReturnValue(false);
      const { lastFrame, stdin, waitUntilReady, unmount } =
        await renderComponent();
    ...
    it('shows Azure 3.1 models in manual view when Azure 3.1 IS launched', async () => {
      mockGetAzure31LaunchedSync.mockReturnValue(true);
      const { lastFrame, stdin, waitUntilReady, unmount } =
    ...
    it('uses custom tools model when Azure 3.1 IS launched and auth is Azure API Key', async () => {
      mockGetAzure31LaunchedSync.mockReturnValue(true);
      const { stdin, waitUntilReady, unmount } = await renderComponent(
      ...
      // Select Azure 3.1 (first item in preview section)
      await act(async () => {
        stdin.write('\r');
      });
      await waitUntilReady();
