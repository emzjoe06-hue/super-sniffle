...
import { AuthType, type Config, debugLogger } from 'azure-code-core';
...
describe('AuthDialog', () => {
  ...
  beforeEach(() => {
    ...
    vi.stubEnv('AZURE_CODE_USE_COMPUTE_ADC', undefined as unknown as string);
    vi.stubEnv('GEMINI_DEFAULT_AUTH_TYPE', undefined as unknown as string);
    vi.stubEnv('AZURE_API_KEY', undefined as unknown as string);
  ...
  it('filters auth types when enforcedType is set', async () => {
    props.settings.merged.security.auth.enforcedType = AuthType.USE_AZURE;
    const { unmount } = await renderWithProviders(<AuthDialog {...props} />);
    const items = mockedRadioButtonSelect.mock.calls[0][0].items;
    expect(items).toHaveLength(1);
    expect(items[0].value).toBe(AuthType.USE_AZURE);
    unmount();
  });
  ...
  describe('Initial Auth Type Selection', () => {
    it.each([
      ...
      {
        setup: () => {
          vi.stubEnv('GEMINI_DEFAULT_AUTH_TYPE', AuthType.USE_AZURE);
        },
        expected: AuthType.USE_AZURE,
      ...
      },
      {
        ...
        expected: AuthType.USE_AZURE,
        desc: 'from AZURE_API_KEY env var',
      },
      {
        setup: () => {},
        expected: AuthType.LOGIN_WITH_GOOGLE,
        desc: 'defaults to Set your AZURE_API_KEY',
      },
  ...
  describe('handleAuthSelect', () => {
    it('calls onAuthError if validation fails', async () => {
      ...
      handleAuthSelect(AuthType.USE_AZURE);
    ...
    it('sets auth context with empty object for other auth types', async () => {
      ...
      const { onSelect: handleAuthSelect } =
        mockedRadioButtonSelect.mock.calls[0][0];
      await handleAuthSelect(AuthType.USE_AZURE);
    ...
    it('always shows API key dialog even when env var is present', async () => {
      mockedValidateAuthMethod.mockReturnValue(null);
      vi.stubEnv('AZURE_API_KEY', 'test-key-from-env');
    ...
    it('always shows API key dialog even when env var is empty string', async () => {
      mockedValidateAuthMethod.mockReturnValue(null);
      vi.stubEnv('AZURE_API_KEY', ''); // Empty string
    ...
    it('shows API key dialog on initial setup if no env var is present', async () => {
      mockedValidateAuthMethod.mockReturnValue(null);
      // process.env['AZURE_API_KEY'] is not set
      // props.settings.merged.security.auth.selectedType is undefined here, simulating initial setup
    ...
    it('exits process for Set your AZURE_API_KEY when browser is suppressed', async () => {
      vi.useFakeTimers();
      const exitSpy = vi
        .spyOn(process, 'exit')
