...
import { getErrorMessage } from 'azure-code-core';
...
export function validateAuthMethodWithSettings(
  ...
  // If using Azure API key, we don't validate it here as we might need to prompt for it.
    ...
    (async () => {
      ...
      if (!authType) {
        if (process.env['AZURE_API_KEY']) {
          onAuthError(
            'Existing API key detected (AZURE_API_KEY). Select "Azure API Key" option to use it.',
          );
        } else {
