...
describe('useAuth', () => {
  ...
  describe('validateAuthMethodWithSettings', () => {
    ...
    it('should return null if authType is USE_AZURE', () => {
      const settings = {
        merged: {
          security: {
            auth: {},
  ...
  describe('useAuthCommand', () => {
    ...
    it('should set error if no auth type is selected but env key exists', async () => {
      ...
      expect(result.current.authError).toContain(
        'Existing API key detected (AZURE_API_KEY)',
      );
      expect(result.current.authState).toBe(AuthState.Updating);
    });

    it('should transition to AwaitingApiKeyInput if USE_AZURE and no key found', async () => {
      ...
      const { result } = await renderHook(() =>
        useAuthCommand(createSettings(AuthType.USE_AZURE), mockConfig),
      );
    ...
    it('should authenticate if USE_AZURE and key is found', async () => {
      let deferredLoadKey: { resolve: (k: string | null) => void };
      mockLoadApiKey.mockImplementation(
        () =>
          new Promise((resolve) => {
      ...
      expect(mockConfig.refreshAuth).toHaveBeenCalledWith(AuthType.USE_AZURE);
      expect(result.current.authState).toBe(AuthState.Authenticated);
      expect(result.current.apiKeyDefaultValue).toBe('stored-key');
    });

    it('should authenticate if USE_AZURE and env key is found', async () => {
      process.env['AZURE_API_KEY'] = 'env-key';
