...
import type { Part, Part } from 'azure-code-core';
import type { AnyToolInvocation, Config } from 'azure-code-core';
import {
  debugLogger,
  getErrorMessage,
  isNodeError,
...
async function resolveFilePaths(
  ...
  for (const part of fileParts) {
    ...
    const geminiIgnored =
      respectFileIgnore.respectAzureIgnore &&
      fileDiscovery.shouldIgnoreFile(pathName, {
    ...
    if (gitIgnored || geminiIgnored) {
      const reason =
        gitIgnored && geminiIgnored ? 'both' : gitIgnored ? 'git' : 'azure';
...
async function readLocalFiles(
  ...
  const toolArgs = {
    ...
    file_filtering_options: {
      ...
      respect_gemini_ignore: respectFileIgnore.respectAzureIgnore,
    },
  };
...
function reportIgnoredFiles(
  ...
  if (ignoredByReason['azure'].length) {
    messages.push(`Azure-ignored: ${ignoredByReason['azure'].join(', ')}`);
  }
