...
import { createMockMessageBus } from 'azure-code-core/src/test-utils/mock-message-bus.js';
...
describe('useToolScheduler', () => {
  ...
  it('preserves responseSubmittedToAzure flag across updates', async () => {
    const { result } = await renderHook(() =>
      useToolScheduler(
        vi.fn().mockResolvedValue(undefined),
        mockConfig,
    ...
    expect(result.current[0][0].responseSubmittedToAzure).toBe(true);
  ...
  it('resolves the schedule promise when scheduler resolves', async () => {
    ...
    const { Scheduler } = await import('azure-code-core');
  ...
  it('setToolCallsForDisplay re-groups tools by schedulerId (Multi-Scheduler support)', async () => {
    ...
    act(() => {
      ...
      setToolCalls((prev) =>
        prev.map((t) => ({ ...t, responseSubmittedToAzure: true })),
      );
    });

    const [toolCalls2] = result.current;
    expect(toolCalls2).toHaveLength(2);
    expect(toolCalls2.every((t) => t.responseSubmittedToAzure)).toBe(true);
  });
