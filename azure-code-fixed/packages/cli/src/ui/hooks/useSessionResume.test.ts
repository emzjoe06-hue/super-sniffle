...
describe('useSessionResume', () => {
  ...
  describe('loadHistoryForResume', () => {
    ...
    it('should not load history if Azure client is not initialized', async () => {
      const { result } = await renderHook(() =>
        useSessionResume({
          ...getDefaultProps(),
          isAzureClientInitialized: false,
        }),
      );
      ...
      expect(mockAzureClient.resumeChat).not.toHaveBeenCalled();
    });
    ...
    it('should handle empty history arrays', async () => {
      ...
      expect(mockHistoryManager.addItem).not.toHaveBeenCalled();
      expect(mockRefreshStatic).toHaveBeenCalledTimes(1);
      expect(mockAzureClient.resumeChat).toHaveBeenCalledWith([], resumedData);
    });
  ...
  describe('automatic resume on mount', () => {
    ...
    it('should not resume when Azure client is not initialized', async () => {
      const conversation: ConversationRecord = {
        sessionId: 'auto-resume-123',
        projectHash: 'project-123',
        startTime: '2025-01-01T00:00:00Z',
    ...
    it('should automatically resume session when resumedSessionData is provided', async () => {
      ...
      expect(mockHistoryManager.addItem).toHaveBeenNthCalledWith(
        2,
        { type: 'azure', text: 'Welcome back!' },
        1,
        true,
      );
      expect(mockRefreshStatic).toHaveBeenCalledTimes(1);
      expect(mockAzureClient.resumeChat).toHaveBeenCalled();
    });
    ...
    it('should convert session messages correctly during auto-resume', async () => {
      ...
      const clientHistory = mockAzureClient.resumeChat.mock.calls[0][0];
