...
import * as core from 'azure-code-core';
...
describe('handleAtCommand', () => {
  ...
  beforeEach(async () => {
    ...
    mockConfig = {
      ...
      getFileFilteringRespectAzureIgnore: () => true,
  ...
  describe('gemini-ignore filtering', () => {
    it('should skip azure-ignored files in @ commands', async () => {
      await createTestFile(
        path.join(testRootDir, AZURE_IGNORE_FILE_NAME),
        'build/output.js',
      );
      ...
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Path ${geminiIgnoredFile} is azure-ignored and will be skipped.`,
      );
      expect(mockOnDebugMessage).toHaveBeenCalledWith(
        `Ignored 1 files:\nAzure-ignored: ${geminiIgnoredFile}`,
      );
    });
  });
  it('should process non-ignored files when .azureignore is present', async () => {
    await createTestFile(
      path.join(testRootDir, AZURE_IGNORE_FILE_NAME),
      'build/output.js',
    );
  ...
  it('should handle mixed azure-ignored and valid files', async () => {
    await createTestFile(
      path.join(testRootDir, AZURE_IGNORE_FILE_NAME),
      'dist/bundle.js',
    );
