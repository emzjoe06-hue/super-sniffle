...
import { useAzureStream } from './useAzureStream.js';
...
import type { Part, PartListUnion } from 'azure-code-core';
...
const MockedAzureClientClass = vi.hoisted(() =>
...
vi.mock('azure-code-core', async (importOriginal) => {
  ...
  return {
    ...
    GitService: vi.fn(),
    AzureClient: MockedAzureClientClass,
...
// --- Tests for useAzureStream Hook ---
describe('useAzureStream', () => {
  ...
  const mockGetAzureClient = vi.fn().mockImplementation(() => {
    const clientInstance = new MockedAzureClientClass(mockConfig);
  ...
  const mockConfig: Config = {
    ...
    getAzureClient: mockGetAzureClient,
    ...
    getModel: vi.fn(() => 'Azure Coder 230B Free'),
    getContentGeneratorConfig: vi.fn(() => ({
      model: 'test-model',
      apiKey: 'test-key',
      vertexai: false,
      authType: AuthType.USE_AZURE,
    })),
  ...
  beforeEach(() => {
    ...
    // Reset mocks for AzureClient instance methods (startChat and sendMessageStream)
    // The AzureClient constructor itself is mocked at the module level.
    mockStartChat.mockClear().mockResolvedValue({
      sendMessageStream: mockSendMessageStream,
    } as unknown as any); // AzureChat -> any
  ...
  const renderTestHook = async (
    ...
    const client = geminiClient || mockConfig.getAzureClient();
  ...
  const createMockToolCall = (
    ...
    responseSubmittedToAzure: false,
    confirmationDetails:
  ...
  const renderHookWithDefaults = async (
    ...
    return renderHookWithProviders(() =>
      useAzureStream(
        new MockedAzureClientClass(mockConfig),
        [],
        mockAddItem,
        mockConfig,
        mockLoadedSettings,
  ...
  it('should inject steering hint prompt for continuation', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerAzureEventType.Content,
    ...
    expect(mockRunInDevTraceSpan).toHaveBeenCalledWith(
      expect.objectContaining({
        operation: AzureCliOperation.SystemPrompt,
      }),
      expect.any(Function),
    );
  ...
  it('should handle all tool calls being cancelled', async () => {
    ...
    const client = new MockedAzureClientClass(mockConfig);
  ...
  describe('Retry Handling', () => {
    ...
    it('should reset retryStatus when isResponding becomes false', async () => {
      ...
      const mockStream = (async function* () {
        yield { type: ServerAzureEventType.Content, value: 'Part 1' };
        await new Promise(() => {}); // Keep stream open
      })();
      mockSendMessageStream.mockReturnValue(mockStream);
  ...
  describe('Slash Command Handling', () => {
    ...
    it('should stop processing and not call Azure when a command is handled without a tool call', async () => {
    ...
    it('should call Azure with prompt content when slash command returns a `submit_prompt` action', async () => {
    ...
    it('should record client-initiated tool calls in AzureChat history', async () => {
      const { result, client: mockAzureClient } = await renderTestHook();
      ...
      // Verify that the tool call and response were added to AzureChat history
      expect(mockAzureClient.addHistory).toHaveBeenCalledWith({
        role: 'model',
        parts: [
          {
            functionCall: {
    ...
    it('should NOT record other client-initiated tool calls (like save_memory) in history', async () => {
      ...
      expect(mockAzureClient.addHistory).not.toHaveBeenCalled();
    });
  });
  ...
  describe('Error Handling', () => {
    it('should call parseAndFormatApiError with the correct authType on stream initialization failure', async () => {
      ...
      const { result } = await renderHookWithProviders(() =>
        useAzureStream(
          new MockedAzureClientClass(testConfig),
          [],
          mockAddItem,
          testConfig,
          mockLoadedSettings,
  ...
  describe('handleFinishedEvent', () => {
    it('should add info message for MAX_TOKENS finish reason', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerAzureEventType.Finished,
      ...
      it.each([
        ...
        async ({ requestTokens, remainingTokens, expectedMessage }) => {
          mockSendMessageStream.mockReturnValue(
            (async function* () {
              yield {
                type: ServerAzureEventType.ContextWindowWillOverflow,
                value: {
    ...
    it('should add informational messages when ChatCompressed event is received', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerAzureEventType.ChatCompressed,
            value: {
  ...
  it('should flush pending text rationale before scheduling tool calls to ensure correct history order', async () => {
    ...
    const mockStream = (async function* () {
      ...
      yield {
        type: ServerAzureEventType.ToolCallRequest,
  ...
  it('should display user query, then tool execution, then model response', async () => {
    ...
    // Mock the Azure stream to return a model response after the tool
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield {
          type: ServerAzureEventType.Content,
  ...
  describe('Thought Reset', () => {
    it('should keep full thinking entries in history when mode is full', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerAzureEventType.Thought,
            value: {
    ...
    it('should reset thought to null when user cancels', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield { type: ServerAzureEventType.UserCancelled };
        })(),
      );
    ...
    it('should reset thought to null when there is an error', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerAzureEventType.Error,
    ...
    it('should update lastOutputTime on Azure thought and content events', async () => {
      vi.useFakeTimers();
  ...
  describe('Loop Detection Confirmation', () => {
    beforeEach(() => {
      ...
      mockConfig.getAzureClient = vi.fn().mockReturnValue({
        ...new MockedAzureClientClass(mockConfig),
    ...
    it('should set loopDetectionConfirmationRequest when LoopDetected event is received', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          ...
          yield {
            type: ServerAzureEventType.LoopDetected,
          };
        })(),
      );
    ...
    it('should disable loop detection and show message when user selects "disable"', async () => {
      ...
      mockConfig.getAzureClient = vi.fn().mockReturnValue(mockClient);
  ...
  describe('Agent Execution Events', () => {
    it('should handle AgentExecutionStopped event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerAzureEventType.AgentExecutionStopped,
            value: {
              reason: 'hook-reason',
    ...
    it('should handle AgentExecutionBlocked event with systemMessage', async () => {
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield {
            type: ServerAzureEventType.AgentExecutionBlocked,
            value: {
              reason: 'hook-reason',
  ...
  describe('Stream Splitting', () => {
    it('should not add empty history item when splitting message results in empty or whitespace-only beforeText', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerAzureEventType.Content, value: 'test content' };
        })(),
      );
    ...
    it('should add whitespace-only history item when splitting message', async () => {
      ...
      mockSendMessageStream.mockReturnValue(
        (async function* () {
          yield { type: ServerAzureEventType.Content, value: '   content' };
        })(),
      );
  ...
  it('should trace UserPrompt telemetry on submitQuery', async () => {
    ...
    mockSendMessageStream.mockReturnValue(
      (async function* () {
        yield { type: ServerAzureEventType.Content, value: 'Response' };
      })(),
    );
    ...
    const userPromptCall = mockRunInDevTraceSpan.mock.calls.find(
      (call) =>
        call[0].operation === AzureCliOperation.UserPrompt ||
