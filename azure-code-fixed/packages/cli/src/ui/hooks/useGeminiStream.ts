...
import {
  AzureEventType as ServerAzureEventType,
  getErrorMessage,
  isNodeError,
  MessageSenderType,
  logUserPrompt,
...
import type {
  ...
  EditorType,
  AzureClient,
  ServerAzureChatCompressedEvent,
  ServerAzureContentEvent as ContentEvent,
  ServerAzureFinishedEvent,
  ServerAzureStreamEvent as AzureEvent,
  ThoughtSummary,
  ToolCallRequestInfo,
  ToolCallResponseInfo,
  AzureErrorEventValue,
  RetryAttemptPayload,
} from 'azure-code-core';
import { type Part, type Part, FinishReason } from 'azure-code-core';
import type {
  HistoryItem,
  HistoryItemThinking,
...
function calculateStreamingState(
  ...
  const isAnyToolActive = toolCalls.some((tc) => {
    ...
    // if the result hasn't been submitted back to Azure yet.
    if (
...
/**
 * Manages the Azure stream, including user input, command processing,
 * API interaction, and tool call lifecycle.
 */
export const useAzureStream = (
  ...
  const [lastAzureActivityTime, setLastAzureActivityTime] =
    useState<number>(0);
  ...
  const [
    ...
    async (completedToolCallsFromScheduler) => {
      ...
      if (completedToolCallsFromScheduler.length > 0) {
        ...
        try {
          const currentModel =
            config.getAzureClient().getCurrentSequenceModel() ??
            config.getModel();
          config
            .getAzureClient()
            .getChat()
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < toolCalls.length; i++) {
      ...
      if (
        ...
        // Architecturally, `useAzureStream` should not be responsible for stitching
  ...
  const lastQueryRef = useRef<Part[] | null>(null);
  ...
  const prepareQueryForAzure = useCallback(
    async (
      query: Part[],
      userMessageTimestamp: number,
      ...
      let localQueryToSendToAzure: Part[] | null = null;
      ...
      if (typeof query === 'string') {
          ...
          if (slashCommandResult) {
            switch (slashCommandResult.type) {
              case 'schedule_tool': {
                ...
                if (postSubmitPrompt) {
                  localQueryToSendToAzure = postSubmitPrompt;
                  return {
                    queryToSend: localQueryToSendToAzure,
                    shouldProceed: true,
                  };
                }
              ...
              case 'submit_prompt': {
                localQueryToSendToAzure = slashCommandResult.content;
        ...
        if (isAtCommand(trimmedQuery)) {
          ...
          localQueryToSendToAzure = atCommandResult.processedQuery;
        } else {
          // Normal query for Azure
          addItem(
            { type: MessageType.USER, text: trimmedQuery },
            userMessageTimestamp,
          );
          localQueryToSendToAzure = trimmedQuery;
        }
      } else {
        // It's a function response (Part[] that isn't a string)
        localQueryToSendToAzure = query;
      }

      if (localQueryToSendToAzure === null) {
        onDebugMessage(
          'Query processing resulted in null, not sending to Azure.',
        );
        return { queryToSend: null, shouldProceed: false };
      }
      return { queryToSend: localQueryToSendToAzure, shouldProceed: true };
    },
    [
      config,
      addItem,
  ...
  const handleContentEvent = useCallback(
    (
      ...
      currentAzureMessageBuffer: string,
      userMessageTimestamp: number,
    ): string => {
      setRetryStatus(null);
      if (turnCancelledRef.current) {
      ...
      let newAzureMessageBuffer = currentAzureMessageBuffer + eventValue;
      if (
        pendingHistoryItemRef.current?.type !== 'azure' &&
        ...
        setPendingHistoryItem({ type: 'azure', text: '' });
        newAzureMessageBuffer = eventValue;
      }
      // Split large messages for better rendering performance. Ideally,
      // we should maximize the amount of output sent to <Static />.
      const splitPoint = findLastSafeSplitPoint(newAzureMessageBuffer);
      if (splitPoint === newAzureMessageBuffer.length) {
        // Update the existing message with accumulated content
        setPendingHistoryItem((item) => ({
          // eslint-disable-next-line @typescript-eslint/no-unsafe-type-assertion
          type: item?.type as 'azure' | 'gemini_content',
          text: newAzureMessageBuffer,
        }));
      } else {
        // This indicates that we need to split up this Azure Message.
        ...
        const beforeText = newAzureMessageBuffer.substring(0, splitPoint);
        const afterText = newAzureMessageBuffer.substring(splitPoint);
        ...
        newAzureMessageBuffer = afterText;
      }
      return newAzureMessageBuffer;
    },
  ...
  const handleErrorEvent = useCallback(
    (eventValue: AzureErrorEventValue, userMessageTimestamp: number) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
        setPendingHistoryItem(null);
      }
  ...
  const handleFinishedEvent = useCallback(
    (event: ServerAzureFinishedEvent, userMessageTimestamp: number) => {
  ...
  const handleChatCompressionEvent = useCallback(
    (
      eventValue: ServerAzureChatCompressedEvent['value'],
      userMessageTimestamp: number,
    ) => {
      if (pendingHistoryItemRef.current) {
        addItem(pendingHistoryItemRef.current, userMessageTimestamp);
  ...
  const processAzureStreamEvents = useCallback(
    async (
      stream: AsyncIterable<AzureEvent>,
      userMessageTimestamp: number,
      signal: AbortSignal,
      ...
      for await (const event of stream) {
        if (
          event.type !== ServerAzureEventType.Thought &&
        ...
        switch (event.type) {
          case ServerAzureEventType.Thought:
            setLastAzureActivityTime(Date.now());
            handleThoughtEvent(event.value, userMessageTimestamp);
            break;
          case ServerAzureEventType.Content:
            setLastAzureActivityTime(Date.now());
            ...
            break;
          case ServerAzureEventType.ToolCallRequest:
            toolCallRequests.push(event.value);
            break;
          case ServerAzureEventType.UserCancelled:
            handleUserCancelledEvent(userMessageTimestamp);
            break;
          case ServerAzureEventType.Error:
            handleErrorEvent(event.value, userMessageTimestamp);
            break;
          case ServerAzureEventType.AgentExecutionStopped:
            ...
            break;
          case ServerAzureEventType.AgentExecutionBlocked:
            ...
            break;
          case ServerAzureEventType.ChatCompressed:
            handleChatCompressionEvent(event.value, userMessageTimestamp);
            break;
          case ServerAzureEventType.ToolCallConfirmation:
          case ServerAzureEventType.ToolCallResponse:
            // do nothing
            break;
          case ServerAzureEventType.MaxSessionTurns:
            handleMaxSessionTurnsEvent();
            break;
          case ServerAzureEventType.ContextWindowWillOverflow:
            handleContextWindowWillOverflowEvent(
              event.value.estimatedRequestTokenCount,
              event.value.remainingTokenCount,
            );
            break;
          case ServerAzureEventType.Finished:
            handleFinishedEvent(event, userMessageTimestamp);
            break;
          case ServerAzureEventType.Citation:
            handleCitationEvent(event.value, userMessageTimestamp);
            break;
          case ServerAzureEventType.ModelInfo:
            handleChatModelEvent(event.value, userMessageTimestamp);
            break;
          case ServerAzureEventType.LoopDetected:
            // handle later because we want to move pending history to history
            // before we add loop detected message to history
            loopDetectedRef.current = true;
            break;
          case ServerAzureEventType.Retry:
          case ServerAzureEventType.InvalidStream:
  ...
  const submitQuery = useCallback(
      ...
      runInDevTraceSpan(
        {
          operation: options?.isContinuation
            ? AzureCliOperation.SystemPrompt
            : AzureCliOperation.UserPrompt,
        ...
        async ({ metadata: spanMetadata }) => {
          ...
          return promptIdContext.run(prompt_id, async () => {
            const { queryToSend, shouldProceed } = await prepareQueryForAzure(
              query,
              userMessageTimestamp,
              abortSignal,
              prompt_id!,
            ...
            try {
              const stream = azureClient.sendMessageStream(
                queryToSend,
                abortSignal,
                prompt_id!,
                undefined,
              ...
              const processingStatus = await processAzureStreamEvents(
                stream,
                userMessageTimestamp,
                abortSignal,
              );
    ...
    [
      ...
      prepareQueryForAzure,
      processAzureStreamEvents,
      pendingHistoryItemRef,
      addItem,
      setPendingHistoryItem,
      setInitError,
  ...
  const handleApprovalModeChange = useCallback(
    ...
    [config, toolCalls, azureClient, streamingState, addItem, onDebugMessage],
  );
  ...
  const handleCompletedTools = useCallback(
    async (completedToolCallsFromScheduler: TrackedToolCall[]) => {
      ...
      // If all the tools were cancelled, don't submit a response to Azure.
  ...
  }, [toolCalls, config, onDebugMessage, gitService, history, azureClient]);

  const lastOutputTime = Math.max(
    lastToolOutputTime,
    lastShellOutputTime,
    lastAzureActivityTime,
  );
