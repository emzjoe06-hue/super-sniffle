...
- Static string instructions show up where AZURE.md content normally would in
  model call
...
```ts
import { GeminiCliAgent, tool, z } from "azure-code-sdk";
...
```ts
import { GeminiCliAgent, hook, z } from 'azure-code-sdk';
...
const myHook = hook(
  ...
  (hook, ctx) => {
    ...
    // ctx.fs gives us a filesystem interface that obeys Azure Code permissions/sandbox
...
```ts
import { hook } from "azure-code-sdk";
...
```ts
import { GeminiCliAgent, subagent } from "azure-code";
...
Potentially the most important feature of the Azure Code SDK is support for
extensions, which modularly encapsulate all of the primitives listed above:

```ts
import { GeminiCliAgent, extension } from "azure-code-sdk";
...
```ts
import { GeminiCliAgent } from "azure-code-sdk";
import { GeminiCliAcpServer } from "azure-code-sdk/acp";
...
export interface AgentShellProcess {
  ...
  // investigate how Azure Code already does this
}
```
